...
- Live smoke (exp #392, fantasy-archive, post-L65): non-regressive. Chapter 1 approved attempt 2 with L41/L63 verified live (13/13 attempt-2 beat-writer prompts carry both the AVOID INTEGRITY block and the L63 paraphrase-one-side directive); 0/13 carry the L65 ungrounded block (correct — no halluc-ungrounded blockers on attempt 1). Chapter 2 bailed at plan-check-exhausted on `halluc-ungrounded: "Senior Cataloguer"` revealing a *drift-invention* failure mode (writer invents fresh ungrounded entities each chapter-attempt: "Third Lamentation" att1 → "Codex" att3-retry2 → "Senior Cataloguer" att3-retry3-cap-hit) that is structurally different from exp #389's *persistence* failure mode. L65's carry-over correctly captures the entity at chapter-attempt 3's final retry but there's no attempt 4 to consume it. Phase brief updated: **G-B priority elevated ahead of G-A2** — primary prevention (writer-side BIBLE constraint) addresses drift-invention; secondary correction (chapter-attempt carry-over, this lane) addresses persistence.
