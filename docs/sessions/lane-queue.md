...
- **Rillgate main-path evidence (L110-L112).** `rillgate-mainpath-1778714560-baseline` drafted repaired source `test-planner-mercenary-rillgate-saltmine-1778674224711` through 10/10 chapters on the production path: 36,997/31,000 words (1.19x), Plan-Assist rows 0, checker blockers 0, planning-context gaps 0, prose-semantic lows 0/40, and high-cap scene-semantic lows 4/166. Compared with `rillgate-full-write-headroom-1778693370`, high-cap scene-semantic evidence is mixed: endpoint landing regressed slightly while scene dramaturgy stayed neutral and world-fact pressure resolved one low. Next harness tuning should target endpoint landing in specific mid-book scenes, functional-state explicit-knowledge warning noise, and lint/integrity repair behavior; keep halluc/semantic judges opt-in.
