...
- Run/thread coherence: manifests, validation, refs, thread maps, static evidence, advisory review, and opt-in context arm are in.
...
- L093: `sceneTurnId` parents and character-context packets are graph-ready/readiness-preserved; v4 cleared
  3-chapter planner smoke and ch1/2/5 paired writes stayed deterministic-clean. Character-context gaps now surface as manual `CHARACTERREF-1`; v6 separates local `requiredCharacterIds` from downstream `affectedCharacterIds`, but 4-chapter planner cohort closed only 9/16 character refs. `diagnostics:corpus-recreation-character-ref-repair` now converts the remaining exact gaps into 7 manual, importable readiness groups covering 10 refs, and accepted candidates can become normal `planning_edit` proposals for the character-ref arrays. Next decide whether to accept those through Plan Readiness or revise the planner contract to reduce the candidate count in a fresh cohort. Treat Option B as lineage fields, not graph implementation.
...
- Corpus structure recreation is active: exact-ID scene-call samples across chapters 1, 2, 5, and 8 held deterministic scene/ID/source-boundary shape. Semantic review found low-signal obligation materiality in chapter 2 despite clean deterministic structure, and `diagnostics:corpus-recreation-readiness` now turns lows into manual Plan Readiness-compatible review groups. `--planner-variant materiality-v1` is HOLD after a 4-chapter follow-up: it cleared chapter-2 lows, but introduced chapter-1 scene-4 motivation/world lows and shortened chapters 1/5. Word counts are advisory warnings, not rewrite gates. Source leakage means literal forbidden source terms/events, not structural similarity. `diagnostics:corpus-recreation-review` builds a read-only static HTML side-by-side review page, and `diagnostics:corpus-recreation-prose-review` adds AI pre-review for prose quality before operator reading. Chapter-2 prose pre-review favored materiality-v1 on payoff propulsion but did not flag failures. Next lever is operator review of the comparison page or a combined materiality + POV-motivation-tradeoff variant if review confirms the same semantic weakness; do not promote materiality-v1.
