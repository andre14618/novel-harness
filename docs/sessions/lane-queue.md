...
- Corpus structure recreation is active: exact-ID scene-call samples across chapters 1, 2, 5, and 8 held deterministic scene/ID/source-boundary shape. Semantic review found low-signal obligation materiality in chapter 2 despite clean deterministic structure, and `diagnostics:corpus-recreation-readiness` turns lows into manual Plan Readiness-compatible review groups. `--planner-variant materiality-v1` is HOLD after clearing chapter-2 lows but introducing chapter-1 lows and shortening chapters 1/5. `causal-materiality-v2` now has a 4-chapter cohort: complete thread refs, 29-row cross-chapter thread map with 0 issues, 85 semantic tasks with 0 lows, and 64 prose tasks with 0 lows. It remains HOLD because it shortened prose heavily (6604/11061 words) and left 8 character-context linkage gaps. Next: convert exact gaps to Plan Readiness items or test a writer expansion/context arm while holding the v2 planner fixed.
