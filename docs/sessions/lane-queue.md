...
- L093: `sceneTurnId` parents and character-context packets are graph-ready/readiness-preserved; v4 cleared
  3-chapter planner smoke and ch1/2/5 paired writes stayed deterministic-clean. Character-context gaps now surface as manual `CHARACTERREF-1` readiness questions, and v5 adds `requiredCharacterIds` plus character-ref closure diagnostics. Treat Option B as lineage fields, not graph implementation.
