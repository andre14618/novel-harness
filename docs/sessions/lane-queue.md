...
- Corpus structure recreation is active: exact-ID scene-call samples across chapters 1, 2, 5, and 8 held deterministic scene/ID/source-boundary shape. `materiality-v1` is HOLD. `causal-materiality-v2` is the current best planner diagnostic: complete thread refs, 29-row cross-chapter thread map with 0 issues, 85 semantic tasks with 0 lows, and 64 prose tasks with 0 lows, but it shortened prose and left 8 character-context linkage gaps. Fixed-plan `thread-context-v1` did not fix length. Default-off `retry-short-scenes-v1` improved the same 4-chapter fixed-plan cohort from 6604/11061 words to 8817/11061, reduced scene-floor misses from 9 to 2, and stayed semantic/prose low-free; it remains diagnostic-only because plan issues and character-context gaps were unchanged. Next: use Plan Readiness for exact character-context gaps or revise the planner contract before further writer promotion.
