...
- **Production writer/planner evidence (2026-05-12).** P1/P2/P3 evidence now
  ...
  scene-contract/adherence mechanics over paid semantic gates. See L106-L108/L112-L114.
...
- **Rillgate main-path evidence (L110-L114).** `rillgate-mainpath-1778714560-baseline` drafted repaired source `test-planner-mercenary-rillgate-saltmine-1778674224711` through 10/10 chapters on the production path: 36,997/31,000 words (1.19x), Plan-Assist rows 0, checker blockers 0, planning-context gaps 0, prose-semantic lows 0/40, and high-cap scene-semantic lows 4/166. Compared with `rillgate-full-write-headroom-1778693370`, high-cap scene-semantic evidence is mixed: endpoint landing regressed slightly while scene dramaturgy stayed neutral and world-fact pressure resolved one low. L113 made LLM style lint auto-fix opt-in and L114 separates checker noise from weight-bearing evidence; next harness tuning should target endpoint landing in specific mid-book scenes while keeping halluc/semantic judges opt-in.
- **Endpoint landing follow-up (2026-05-13/14).** `rillgate-endpoint-landing-1778717740-production-path` added a true source-preserving production-path isolated run: source seed writer overrides were preserved, tight-anchored drafting brief was enabled on 40/40 writer events, and chapters 1-6 drafted at 18,396/18,600 words (0.989x). Endpoint lows stayed 4/30, pointing upstream to endpoint-contract quality. Follow-up hygiene changed `SCENE-ENDPOINT-DUPLICATE` readiness to scalar `consequence` edits, repaired the three chapter 4 duplicate endpoint defects through reviewed `planning_edit`, and added clone-only `--chapter-start` redraft windows. Narrow chapter 4 redraft `rillgate-ch4-endpoint-hygiene-redraft-1778758843-production-path` completed at 2810/3100 words with Plan-Assist 0, checker readiness 0, prose lows 0/4, scene-contract coverage 5/5, and scene-semantic lows 1/14. Semantic adjudication then confirmed the remaining scene 3 action-level endpoint low was real enough to repair upstream, applied a scalar `consequence` planning edit, and redrafted `rillgate-ch4-endpoint-action-redraft-1778760152-production-path` at 2852/3100 words with Plan-Assist 0, checker readiness 0, prose lows 0/4, and scene-semantic lows 0/14. Endpoint-only scene-semantic lows now convert to scalar `consequence` readiness candidates, and planner prompts require observable executable consequence rather than implied intent; do not add deterministic endpoint-word rules. See `docs/sessions/2026-05-14-endpoint-contract-hygiene.md`.
- **Plan-state consistency readiness (2026-05-14, L115).** `rillgate-coherent-prod-1778761633-production-path` drafted all 10 chapters from repaired source `rillgate-ch4-endpoint-hygiene-1778723371`: 31,676/31,000 words (1.022x), Plan-Assist 0, prose-semantic lows 0/40, scene-semantic lows 4/166, scene readiness import inserted 4, and checker readiness found 2 weight-bearing continuity blockers in chapter 9. L115 now adds optional `factStatus`, grouped same-target checker-readiness import, production-clean telemetry for weight-bearing checker rows, and a live DeepSeek Flash adjacent-chapter source audit. Latest source audit has 3 open high/medium plan-state readiness items after staling 8 prior same-import rows: chapter 3-to-4 crew handoff, chapter 5-to-6 Kael physical state, and chapter 8-to-9 split/together handoff. Next work is reviewing/applying those source readiness items before another coherent write. See `docs/sessions/2026-05-14-coherent-production-draft.md` and `output/plan-state-consistency/rillgate-ch4-endpoint-hygiene-1778723371/source/`.
