...
- **Endpoint landing follow-up (2026-05-13/14).** `rillgate-endpoint-landing-1778717740-production-path` added a true source-preserving production-path isolated run: source seed writer overrides were preserved, tight-anchored drafting brief was enabled on 40/40 writer events, and chapters 1-6 drafted at 18,396/18,600 words (0.989x). Endpoint lows stayed 4/30: ch3 and ch5 old lows resolved, but ch2 scene 1 and ch4 scene 3 regressed while ch4 scene 1/ch6 scene 2 persisted. Root shape is upstream endpoint-contract quality, not missing downstream context: description/outcome mismatches, duplicated outcome/consequence/turningPoint, and cognitive-only landings. Shipped prompt/telemetry improvements and a selective-scene-turn retry guard for duplicate endpoint fields; next evidence should re-plan or Plan Readiness-repair those exact scenes before another drafting pass.
