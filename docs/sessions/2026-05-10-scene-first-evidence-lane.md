...
### Smoke #3 — production-sent prompt audit

To verify the no-LLM check is faithful to what production actually sends to V4 Flash, queried the `llm_calls.user_prompt` row that the baseline arm of `b1-smoke-1778420126-baseline` produced for ch1 beat 0 before the run was killed:

```
length: 6496 chars      (matches no-LLM raw-arm prompt size exactly)
contains "Chapter ID: ch-001-wound":     true
contains "Beat ID: ch-001-wound":        true
contains "[char_sylvie_dunmore]":        true
contains "[char_corporal_jien]":         true
contains "Mode: thread-character-context-v1": true
```

Triple-confirmation of the ablation surface: unit tests prove the renderer's contract; the no-LLM `buildBeatContext` integration check proves the seed → resolver → renderer plumbing; the production prompt audit proves the no-LLM check faithfully reproduces what V4 Flash actually receives. The id-suppress arm's prompt for the same beat is byte-identical to the no-LLM suppress-arm output (6335 chars, all four Cluster-1 markers absent, trace metadata preserved). No further LLM smoke is needed to ship the ablation flag.
