...
## Currently in flight (2026-04-30 ~08:00 UTC)

| Work | Status |
|---|---|
| LXC phase-eval probe on beat-expansion variant `abcd78f` | Earlier run failed at 07:42 UTC on a prompt-schema enum mismatch (planner emitted `miceActive=['E','C']` against schema enum `['I']`). Fix shipped `0c8457d`. Re-deploy + re-run pending. |
