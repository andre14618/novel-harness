...
**Ongoing:** `coverage-balanced` has one clean screen-pass on `fantasy-system-heretic`; rerun on at least one additional seed before considering default-prompt changes. Keep auto-repair policy unchanged until the split-mapper sample set shows stable low retry/repair rates.
...
**Mechanism:**
- `src/harness/ids.ts` provides `enrichOutlineIds(outline)` — idempotent, mutates in place, assigns missing IDs from content (chapter from `chapterNumber + title`, beat from `chapterId + beatIndex + description`, character from name slug, knowledge/state from `characterId + content slug`), and resolves obligation `sourceId` references by exact-text match against fact/knowledge/state registries.
- The planning loop runs enrichment after `mergeStateMapping`, after auto-repair, and inside `validateBeatObligationCoverage` to guarantee every outline that flows past planning carries the contract.
...
- Auto-repair (`repairBeatObligationCoverage`) builds replacement obligations with `sourceId`, `sourceKind`, `characterId`, and `obligationId` set; placement is the only fuzzy decision (beat-text match chooses where, never whether).
...
- `scripts/phase-eval/print-screen-verdict.ts` G1 for the state-mapper metric set is now `missing_source_ids = 0 AND unknown_source_ids = 0 AND duplicate_source_ids = 0 AND character_id_mismatches = 0`. `implicit_text_matches` is exposed as a diagnostic counter; it never feeds the verdict.
...
- **Require LLM-emitted IDs only (no deterministic fallback).** Until the prompt change soaks, every outline would fail validation; deterministic backfill via text-match-then-link gives a smooth transition while keeping the contract's pass/fail purely ID-based.
...
- Lessons-learned addition: fuzzy semantic matching is a derivation pathway (code can use it to *propose* a link), but it must never be a contract validation pathway.
