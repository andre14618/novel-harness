...
| --- | --- | --- | --- | --- |
| L115 | 2026-05-14 | active | Production source hygiene now includes adjacent-chapter plan-state consistency readiness: state facts may carry optional `factStatus`, live DeepSeek Flash handoff diagnostics emit deterministic chapter/scene repair targets before drafting, checker readiness preserves grouped same-target evidence, and weight-bearing checker rows affect production-clean telemetry without becoming noisy hard drafting blockers. | `docs/decisions/L115-plan-state-consistency-readiness.md` |
