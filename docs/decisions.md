...
| L116 | 2026-05-14 | active | Endpoint scene-semantic lows now distinguish unresolved-choice/pressure endpoints from real misses: an offer, forced consideration, or valid follow-through hook can satisfy endpoint landing when the scene contract declares that pressure rather than a resolved choice. The Rillgate five-low adjudication marked four endpoint rows false-positive/not-applicable and isolated one real ch9 world-fact miss later repaired under L117. | `docs/decisions/L116-endpoint-semantic-low-adjudication.md` |
| L115 | 2026-05-14 | active | Production source hygiene now includes adjacent-chapter plan-state consistency readiness: state facts may carry optional `factStatus`, live DeepSeek Flash handoff diagnostics emit deterministic chapter/scene repair targets before drafting, next-chapter output facts/state are excluded from opening handoff context, scene `opposition` is repairable, and checker readiness separates production-clean signal from noisy hard drafting blockers. | `docs/decisions/L115-plan-state-consistency-readiness.md` |
| L114 | 2026-05-14 | active | Checker telemetry now separates raw findings from weight-bearing evidence: negative standard blockers can drive default readiness/semantic-gate pressure, while positive, low-confidence, ambiguous, and explicitness-only rows remain visible as advisory/noise. | `docs/decisions/L114-weight-bearing-checker-telemetry.md` |
| L113 | 2026-05-14 | active | Production drafting keeps deterministic lint detection telemetry and prose-integrity repair/checking, but automatic LLM style lint repair is default-off behind `lintAutoFixEnabled` because recent runs showed repair-induced fused-boundary and quote-integrity defects. | `docs/decisions/L113-lint-detect-only-default.md` |
