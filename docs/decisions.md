...
### L57 — OpenCode review of Phase 4 + Phase 5 collaborative-proposal bundle: 12-commit fix sweep (2026-05-04, exp #453-464)

**Decision:** Address every finding from the OpenCode + GPT review of `7f41833^..b3160ab` (the Phase 4 commits 1-4 + Phase 5 commits 1-3 series) as one-finding-per-commit, ordered by reviewer-recommended dependency: HIGH 1 (lock contract) → MEDIUM C (atomic reads) → HIGH 2 (v2 inputs) → MEDIUM A/B/F/G/H1/H2 (envelope/UI/payload) → LOW 1/2/3 (defence-in-depth). Defer the docs sweep until the entire bundle lands, then close all `docs-impact: pending` markers in one paired sweep.

**Why:** Review verdict was RED with 2 HIGH + 7 MEDIUM + 3 LOW. The two HIGHs were both load-bearing on the lock contract: HIGH 1 ("trust the client hash") meant a stale-tab operator could lock against drift they couldn't see; HIGH 2 ("v1 inputs miss the planning graph") meant v1 hashes were silently agreeing across diverging planning states because world systems / cultures / character cultures / character-system awareness were all writer-context inputs that were not in the v1 hash set. Both HIGHs invalidated the lock-as-evidence guarantee that Phase 4 was supposed to deliver; ignoring them and shipping Phase 5/6 on that foundation would have meant rebuilding the substrate later under more pressure.

**Per-finding fix shape (commit / experiment / diff scope):**

- **HIGH 1 — recompute-and-compare lock contract** (`08de294` / exp #453 / `src/orchestrator/planning-snapshot-routes.ts` + `PlanningSnapshotPage.tsx`). The route now calls `computePlanningSnapshotHash(novelId, SNAPSHOT_VERSION)` itself and rejects body.hash mismatch with `409 + { expectedHash, providedHash }`. UI surfaces both hashes truncated to 16 chars on the lock-failure card with a Refresh hint. Header docstring updated.
- **HIGH 2 — v2 snapshot inputs** (`bd65fd8` / exp #455 / `src/canon/planning-snapshot.ts`). `SNAPSHOT_VERSION = "v2"`; v2 reads add `readWorldSystemsOrdered` / `readCulturesOrdered` / `readCharacterCulturesOrdered` / `readCharacterSystemAwarenessOrdered`. `PlanningSnapshotVersion = "v1" | "v2"`, default v2. `computePlanningSnapshotHashFromInputs(inputs, version)` branches; v1 stays available for back-compat. The version byte in the digest makes id collisions across versions impossible.
- **MEDIUM A — concurrent-insert retry** (`b28256b` / exp #458 / `src/orchestrator/proposal-envelope-routes.ts`). When `updateEnvelopeResolution` returns 0 rows AND `findEnvelopeById` finds the row in `pending`, retry the update inside the same transaction; throw `alreadyResolved` on second 0-row response. Closes the window where two concurrent inserts of the same envelope id let one resolve win and the other silently lose its decision.
- **MEDIUM B — self-parent (1-cycle) guard** (`6075380` / exp #461 / `src/canon/proposal-envelope.ts` + `src/canon/editorial-proposal.ts`). All three envelope builders throw if `parentEnvelopeId === computedId`. Test fixture for `editorial-flag parentEnvelopeId round-trips through the column` re-pointed to use an unrelated parent string — the DB-level invariant under test (ON CONFLICT preserves original lineage) is unchanged; the construction shape is what changed.
- **MEDIUM C — atomic snapshot reads** (`4e0b500` / exp #454 / `src/canon/planning-snapshot.ts`). All snapshot reads moved into `db.begin(tx => …)` with an `Executor` thread-through. Previously the four accessors ran on the bare `db` connection, opening a window where `getCurrentPlanningSnapshot` could observe a half-applied multi-statement planning edit. Also removed two silent `.catch()` swallows that masked transient DB errors.
- **MEDIUM F — share canonicalize/stableHash helper** (`c794ad4` / exp #457 / `src/canon/editorial-proposal.ts`). Replaced local `canonicalize` + `stableHashHex` with `import { stableHash } from "./proposal-envelope"`. The reviewer's "circular import" claim was wrong (the helper module has no canon-domain imports); just extracted the shared helper directly.
- **MEDIUM G — AdjustPanel novel-switch state isolation** (`ada62ea` / exp #456 / `ui/src/components/ArtifactPreviews.tsx`). `seededNovelIdRef` tracks displayed novelId across effect runs; on novelId change, unconditionally reset turns/pendingPatches/envelopes/envelopeStates/input/historyEnvelopes/historyError before seed fetch; inside async fetch, re-check ref against active novel before applying. Fixes the fast-switch race where prior-novel rows leaked.
- **MEDIUM H1 — beat-coverage strict validation** (`a2ead0f` / exp #460 / `src/canon/editorial-beat-coverage.ts`). `beatCoverageVerdictSchema` refined to require `evidenceQuote` (non-empty) when `covered=true`. New `validateBeatCoverageLlmOutput(output, outline)` post-validator catches missing / duplicate / out-of-range beatIndex against the actual outline. The schema now refuses LLM outputs that claim coverage without grounding it in a quote.
- **MEDIUM H2 — chapterRef caller-supplied** (`30dfadd` / exp #459 / `src/canon/editorial-beat-coverage.ts`). `BuildBeatCoverageProposalsArgs` requires `chapterRef` from the caller; envelope `target.ref` and payload `chapterRef` unify against one source of truth instead of being inferred from the outline. Removed the `outline_chapter_ref(outline)` helper.
- **LOW 1 — DB-level one-way lock trigger** (`5289ab0` / exp #464 / `sql/039_planning_snapshots_lock_immutability.sql` + `src/db/planning-snapshots.test.ts`). BEFORE UPDATE trigger raises `check_violation` if any of `locked_at`, `locked_by_kind`, `locked_by_ref`, `locked_note` changes on an already-locked row. First lock transition (NULL → non-NULL) still allowed; DELETE intentionally unblocked so test cleanup / orphan reaping keeps working. Three new tests pin positive (first lock allowed) + negative (post-lock mutation blocked, locked_at clearing blocked) sides.
- **LOW 2 — disable Re-lock when locked-clean** (`1e5f9a5` / exp #462 / `ui/src/components/PlanningSnapshotPage.tsx`). Button disabled + relabeled "Locked (no action)" with a tooltip when `lockedSnapshot != null && !drift`. The same-hash re-lock fails the `WHERE locked_at IS NULL` predicate by construction, so the operator's only meaningful next action is Refresh (or planning edits → drift); making the no-op nature visible avoids the guaranteed-409 trap.
- **LOW 3 — editorial list helpers expose truncation** (`428b7cc` / exp #463 / `src/db/editorial-envelopes.ts`). `listEditorialFlagEnvelopes` / `listProseEditEnvelopes` return `{ envelopes, hasMore }` via over-fetch-by-1. No current consumer needs cursor-based pagination, but the cheap helper change keeps the door open and gives any future UI surface a correct truncation signal.

**Alternatives rejected:**
- *Bundle multiple findings per commit.* Rejected — CLAUDE.md autonomous-loop rule explicitly says one-finding-per-commit for Codex/review re-review items, and the per-commit experiment row gives each fix its own audit trail.
- *Defer LOWs (especially LOW 1 / LOW 3) until a UI consumer exists.* Rejected for LOW 1 — the trigger is cheap and closes a real defence-in-depth gap on the lock contract that matters for Phase 6's policy engine. Compromised on LOW 3 — kept the helper change but did not ship the cursor-based pagination consumer.
- *Defer the v2 snapshot expansion (HIGH 2) and just close HIGH 1.* Rejected — HIGH 2 was the load-bearing finding: if v1 hashes silently agree across diverging planning states, the lock-as-evidence guarantee is a lie even with the recompute-and-compare contract. Both HIGHs had to land together.

**Tests:**
- All 12 commits ship with at least one new test pinning the fix. Aggregate after the bundle: 81 pass / 204 expects across `editorial-proposal.test.ts` + `proposal-envelope.test.ts` + `editorial-beat-coverage.test.ts` + `planning-snapshot.test.ts`; 13 pass / 68 expects on `editorial-envelopes.test.ts`; 12 pass / 35 expects on `planning-snapshots.test.ts` (3 new + 9 pre-existing).
- Sweep across `src/canon` + `src/db` + `src/orchestrator`: 402 pass / 1 fail in `canon-proposal-routes.test.ts > POST bulk-resolve` (timeout flake — confirmed pre-existing via `git stash` counterfactual on prior HEAD; not introduced by the bundle).
- Server `tsc --noEmit` clean. UI `tsc + vite build` clean.

**Documentation:** `docs/current-state.md` Current Session 2026-05-04 entry summarizes the 12 commits with experiment IDs. `docs/sessions/lane-queue.md` §Completed gains an entry. All 12 commits' `docs-impact: pending` markers are closed by this paired sweep.
...
### L61 — Phase 6 commits 1-3: ApprovalPolicy schema + evaluator + audit-trail integration on artifact_patch + prose_edit (2026-05-04, exp #470-472)

**Decision:** Open the Phase 6 lane (Approval Policy Engine) by shipping three coherent commits: (1) pure schema + deterministic evaluator (tracer-bullet, no DB, no route integration); (2) wire `evaluatePolicy` into the `artifact_patch` resolve route + audit-trail columns + manual-v1 default; (3) mirror commit 2 on the `prose_edit` resolve route. The operator's `status` still drives what applies; the policy fields are an audit-trail surface only. A future commit will add an autonomous decide path that lets the policy fire the apply directly.

**Why:** The collaborative-proposal-workflow design's Phase 6 acceptance criteria are "(a) same proposal can be processed manually or by policy; (b) policy cannot auto-approve high-risk semantic Canon proposals unless an explicit future gate enables it; (c) eval mode logs what it would have done without mutating committed Canon." Criterion (a) requires a uniform policy evaluator across all four envelope kinds; (b) requires `manualKinds` defaulting to `["canon_update"]`; (c) requires an `eval` mode that returns `{ decision: "shadow", shadowOf: <innerDecision> }`. These three commits ship the evaluator and start landing the per-route audit-trail integrations that Phase 7's replay metrics will feed off of.

**Schema (commit 1, `96b16fc`, exp #470):** `ApprovalPolicy` carries `version` (opaque string) + `mode` (`"manual" | "assisted" | "autonomous" | "eval"`) + optional `autoApproveRiskCeiling` (default `"low"`) + optional `manualKinds` (default `["canon_update"]`). `RISK_RANK` is `mechanical=0 < low=1 < medium=2 < high=3`. `PolicyEvaluation` is `{ decision: "queue" | "approve" | "reject" | "shadow", policyVersion, reasons: string[], shadowOf?: PolicyDecision }`. The evaluator is total — every envelope produces a decision; there's no "policy doesn't apply" path. Mode-specific rules:

- **manual** queues every proposal regardless of risk or kind.
- **assisted** auto-approves only `prose_edit` envelopes at `risk: "mechanical"`. Everything else queues — the design's "deterministic mechanical prose fixes only" safe default.
- **autonomous** auto-approves any kind whose risk is `<= autoApproveRiskCeiling`, UNLESS the kind is in `manualKinds`. Producer `policyRecommendation.decision === "reject"` overrides every approve path.
- **eval** never mutates; recursively evaluates as autonomous, then returns `{ decision: "shadow", shadowOf: <innerDecision>, reasons: ["mode=eval; would-have=<inner>", ...inner.reasons] }`.

**Precedence:** `manualKinds` is checked BEFORE producer-reject. Rationale: rejecting Canon (or any other manual-kind envelope) without an operator's eyes loses information about the producer's reasoning. Manual-kind envelopes always queue — even if the producer recommends reject, the operator gets to read the producer's reasoning and decide.

**Route integration (commits 2 + 3, `745046b` + `115c159`, exp #471 + #472):** New nullable columns on `proposal_envelopes` (`sql/040_proposal_envelopes_policy_resolution.sql`):

- `resolution_policy_decision` — what the policy engine decided.
- `resolution_policy_version` — the opaque policy version string.
- `resolution_policy_reasons` — JSONB array of reason strings.

Distinct from the existing `policy_decision` + `policy_reasons` columns, which capture the producer's `policyRecommendation` (a producer-side signal about its own confidence). The new `resolution_*` columns capture the policy ENGINE's evaluation at resolve time. Both surface in the audit trail. NULL columns on rows resolved before this migration; the resolve routes fill them on every new resolution.

`updateEnvelopeResolution` extended to take optional `policyDecision` / `policyVersion` / `policyReasons`. The resolve routes (artifact_patch in commit 2, prose_edit in commit 3) accept optional `policy: ApprovalPolicy` in body; default `{ version: "manual-v1", mode: "manual" }` when omitted. The version string is opaque; pinning "manual-v1" lets audit consumers distinguish "no policy attached" (NULL) from "explicit manual default" (the audit row carries decision=queue/version=manual-v1).

The success response (rejected and applied) carries `policy: { decision, version }` so callers can verify the evaluation without a DB round-trip. This is what the future autonomous decide endpoint will use to confirm "policy decided X, route applied X."

**Pre-existing schema bug fixed (commit 2):** While threading `ProposalEnvelopeStatus` through the route's zod schema, discovered the `policyRecommendationSchema` had a stale enum (`["queue", "shadow", "auto-apply"]`) that didn't match the canonical `"queue" | "approve" | "reject" | "shadow"` from `ProposalPolicyRecommendation`. The previous shape would have rejected producers that recommend `approve` or `reject`. Fixed in this commit (corrected enum + added optional `policyVersion` field).

**Alternatives rejected:**

- *Have the policy drive the apply directly in commits 2-3 (not just record).* Rejected — the audit-trail piece is the smaller, lower-risk step; landing it first means Phase 7's replay metrics can start running over real data immediately, and the autonomous decide endpoint becomes a thin wrapper that converts a policy `approve` decision into the existing operator-approve path. Splitting the work also keeps each commit reviewable.
- *Record the policy on a separate audit table.* Rejected — the resolution event is one tx, the audit fields belong on the same row. No data-integrity advantage to a separate table; just an extra join cost on every audit query.
- *Make the policy a server-side singleton loaded from env or DB.* Rejected for v1 — caller-supplied policy in the request body keeps the route stateless and lets autonomous test mode (Phase 6 acceptance) drive any policy without changing server config. Per-novel persisted policy is a Phase 6 commit-5+ concern.
- *Auto-apply on operator status with policy-decision overlay (caller passes policy, but route ignores status if policy disagrees).* Rejected — the operator's intentional override is a load-bearing audit signal. "Operator approved despite policy=queue" tells Phase 7 the operator disagreed with the policy on this specific case; we want that signal recorded, not silenced.

**Tests:** 23 evaluator tests / 53 expects (commit 1) — manual queues all, assisted only `prose_edit @ mechanical`, autonomous threshold ordering with default + explicit ceilings, manualKinds (default block + opt-out + custom), producer-reject override across modes, manualKinds-precedence over reject, eval mode shadow behavior, policyVersion always surfaces. 33 artifact_patch route tests / 154 expects (commit 2 — +6 over baseline). 17 prose_edit route tests / 80 expects (commit 3 — +4 over baseline). Sweep 475/476 across canon + db + orchestrator (1 pre-existing canon-proposal-routes bulk-resolve flake unchanged — confirmed unrelated via `git stash` counterfactual on baseline). Server tsc clean. Migration applied locally + LXC; orchestrator restarted clean.

**Documentation:** `docs/current-state.md` Current Session 2026-05-04 gains a Phase 6 commits 1-3 bullet. `docs/sessions/lane-queue.md` Phase 6 §Next entry updated to "commits 1-3 DONE; commits 4-5 next"; §Completed gains three entries. All three commits' `docs-impact: pending` markers closed by this paired sweep.

**Ongoing implications:**

- **Phase 6 is now in flight on a sound foundation.** The evaluator is total, deterministic, and pure (no DB, no LLM). The `artifact_patch` and `prose_edit` resolve paths now record policy decisions on every resolution. Phase 7's replay harness can already start computing autonomy metrics on these two kinds.
- **Phase 6 commit 4** is canon_proposals integration. Different table (`canon_proposals` not `proposal_envelopes`); needs its own migration. Risk classification for canon_update is design-blocked — `manualKinds` default of `["canon_update"]` ensures these always queue regardless, but the audit-trail signal still matters for replay metrics. Construct a synthetic envelope inline (kind=canon_update, risk=high default) when feeding `evaluatePolicy`.
- **Phase 6 commit 5** is the autonomous decide endpoint. Take a policy + an envelope id, evaluate, dispatch to the existing apply path with `resolved_by_kind: "policy"`. This is what makes autonomous mode actually fire applies without operator clicks. Approve and reject cases both wrap the existing routes; `queue` returns 200 + `{ decision: "queue" }` without mutating; `shadow` records the would-have decision without mutating.
- **No safe-default surprises.** The default manual-v1 policy is the conservative choice — every proposal queues. Callers must explicitly opt into autonomous/assisted/eval. This matches the design's "manual is the safe default" and means existing operators (manual mode) see no behavior change from Phase 6 commits 2-3.
- **`policyRecommendationSchema` was wrong for ~3 weeks** (since Phase 3 commit 1 landed it with `["queue", "shadow", "auto-apply"]`). Producers that should have recommended `approve` or `reject` would have been rejected by the route's body validator. The bug was never caught because no producer in the runtime today emits anything other than `queue` for the producer recommendation field — the schema was over-constrained but never exercised at the limit. Fixed defensively as part of commit 2.
### L62 — Phase 6 fully closed: ApprovalPolicy engine drives autonomous applies end-to-end (2026-05-04, exp #473-474)

**Decision:** Close the Phase 6 lane (Approval Policy Engine) by shipping commits 4 + 5 after the §L61 commits 1-3 audit-trail wiring. Commit 5 (`a4853d8`) ships the autonomous decide endpoint that lets the policy actually fire applies on artifact_patch + prose_edit envelopes; commit 4 (`220e874`) lands the parallel audit-trail integration on the canon_proposals table. With both, all four envelope kinds record policy decisions on every resolution, and two of four kinds support fully autonomous applies.

**Why:** The collaborative-proposal-workflow design's Phase 6 acceptance criteria are "(a) same proposal can be processed manually or by policy; (b) policy cannot auto-approve high-risk semantic Canon proposals unless an explicit future gate enables it; (c) eval mode logs what it would have done without mutating committed Canon." §L61's commits 1-3 added the evaluator and audit-trail recording but kept the operator's `status` as the load-bearing decision driver. (a) requires an active path where the policy itself drives the apply — that's commit 5. (c) requires a no-mutation eval-mode dispatch that records `status="shadowed"` — also commit 5. (b) is enforced by `manualKinds: ["canon_update"]` default in the evaluator (commit 1) plus the canon_proposals route synthesizing a `kind="canon_update"` envelope so the manualKinds check fires (commit 4).

**Active dispatch (commit 5, `a4853d8`, exp #473):** New endpoint `POST /api/novel/:novelId/proposal-envelopes/:envelopeId/policy-decide` with body `{ policy: ApprovalPolicy, operatorNote?: string }`. Loads the envelope by id from `proposal_envelopes`, coerces by `kind` (artifact_patch / prose_edit supported; canon_update + editorial_flag → 422 with redirect message), evaluates the active policy, dispatches by `evaluation.decision`:

- `queue` → 200 + reasons; no mutation. Operator decides this one manually.
- `shadow` → records `status="shadowed"` + `resolved_by_kind="policy"` + audit fields; no apply. The shadowed status keeps the row out of the pending list while preserving evidence for replay.
- `approve`/`reject` → re-issues the existing kind-specific resolve route (`/proposal-envelopes/resolve` or `/prose-edits/resolve`) with `status` mapped from decision, `resolvedBy="policy"`, and the same policy. The resolve route's atomic compare-and-apply tx handles the actual apply under SELECT FOR UPDATE; the audit row gets the same policy fields the manual path would.

The dispatch via Request reissue is intentional. The existing resolve routes carry the concurrent-resolve race retry, hash-precondition check inside the tx, FOR UPDATE locking. Re-issuing a Request with `resolvedBy="policy"` reuses every guard. The cost is one extra zod parse on the constructed body. The benefit is no risk of subtle drift between the manual and autonomous apply paths — they ARE the same path.

**Existing schema extensions:** Both `proposal-envelope-routes.ts` and `prose-edit-routes.ts` resolve schemas now accept optional `resolvedBy: "human" | "policy" | "script" | "test"` (default `"human"`). The new policy-decide route passes `"policy"` so the audit row's `resolved_by_kind` distinguishes autonomous applies from operator clicks. No behavior change for existing manual callers — the field is optional with the right default.

**Canon-proposals integration (commit 4, `220e874`, exp #474):** Migration `sql/041_canon_proposals_policy_resolution.sql` adds the same audit columns as Phase 6 commit 2's sql/040, plus `resolved_by_kind` (new for canon_proposals; existing on `proposal_envelopes` since Phase 3 commit 4). `CanonSubstrate.resolveProposal` opts extended with policy + resolvedByKind. Both adapters threaded: `InMemoryCanonSubstrate` accepts but ignores (in-memory has no audit table; substrate equivalence tests pass), `PostgresCanonSubstrate` writes via `updateProposalResolution` inside the existing canon-commit tx.

The canon-proposal-routes synthesizes a minimal `ReviewProposalEnvelope` (kind=`canon_update`, risk=`high`, neutral `policyRecommendation`) to feed `evaluatePolicy`. The `manualKinds` default makes canon always queue regardless of mode unless callers explicitly opt out via empty `manualKinds`. The audit signal still matters: NULL means "no policy attached", non-NULL means "policy was evaluated and said X". Operators get a clean diagnostic for whether a row went through Phase 6 evaluation.

**Alternatives rejected:**

- *Build a separate autonomous-mode runtime that doesn't share routes with manual.* Rejected — this was the design's original temptation and explicitly retired in the design doc's "Autonomous mode should use the same proposal mechanics with a different approval policy, not a separate pipeline." Sharing routes via Request reissue keeps the apply path single-source-of-truth and means autonomous mode inherits every existing safety check by construction.
- *Factor the apply logic out of the existing resolve routes into shared helpers.* Rejected for v1 — the existing routes are tested and stable with multi-commit coverage on the concurrent-resolve race + hash-precondition contract. A factoring refactor before commit 5 lands would add risk without behavior benefit. We can refactor later if a third caller path emerges.
- *Make the canon-proposals route call the new policy-decide endpoint internally.* Rejected — canon_proposals lives on a different table (different from `proposal_envelopes`); the active dispatch's "load envelope by id" wouldn't find canon proposals. The cleaner shape is to record the audit on the canon route directly (commit 4) and leave the unified policy-decide for the envelope-table kinds. Manual canon decisions still go through `/canon-proposals/:id/resolve` and now record policy fields too.
- *Hardcode autonomous mode as a server-side singleton.* Rejected — caller-supplied policy in the request body keeps the route stateless and lets autonomous test mode (Phase 6 acceptance) drive any policy without changing server config. Per-novel persisted policy is a follow-on commit.

**Tests:** 13 policy-decide tests / 70 expects (commit 5) covering all four decisions × both supported kinds + error paths (404 missing, 400 novelId mismatch, 409 already-resolved, 422 unsupported kind, 400 missing-policy). The load-bearing demonstrability test pins: an autonomous policy with default ceiling=low fires the apply on a low-risk artifact_patch envelope WITHOUT operator intervention; the audit row carries `resolved_by_kind="policy"`, `resolution_policy_decision="approve"`, `resolution_policy_version="auto-v1"`. 5 canon-proposal Phase 6 tests / 18 expects (commit 4). Substrate equivalence + db substrate tests 103/103 / 429 expects. Sweep 494/494 across canon + db + orchestrator (clean run; the previously-flaking canon-proposal-routes bulk-resolve test passed clean — flake confirmed intermittent across multiple runs but unrelated to Phase 6 changes by construction).

**Documentation:** `docs/current-state.md` Current Session 2026-05-04 gains a "Phase 6 fully closed" composite bullet (replacing the partial commits 1-3 entry from §L61). `docs/sessions/lane-queue.md` Phase 6 §Next entry advances to "fully done"; §Completed gains an entry for commits 4-5. All five commits' `docs-impact: pending` markers closed.

**Ongoing implications:**

- **Phase 6 of the collaborative-proposal-workflow design is fully done.** Schema (1) + artifact_patch policy (2) + prose_edit policy (3) + canon_proposals policy (4) + autonomous decide endpoint (5) all shipped. The autonomous-mode acceptance criterion ("same proposal can be processed manually or by policy") is now demonstrated end-to-end on artifact_patch + prose_edit. Canon proposals always queue by manualKinds default — that's the design's "manual for Canon" safe default; opt-out is available via empty `manualKinds`.
- **Phase 7 (Autonomous Evaluation Loop) is now unblocked.** All four envelope kinds record policy decisions on every resolution. A replay harness over `(resolution_policy_decision, status, resolved_by_kind)` rows can compute precision-by-kind, intervention rate, and policy-vs-operator agreement. The replay harness implementation is the Phase 7 lane.
- **Producer `policyRecommendation` is now meaningful.** Producers (LLM editorial modules, lint converter, planner) can self-report `decision: "reject"` to override every approve path the policy would otherwise return. The schema is now correct (canonical `"queue"|"approve"|"reject"|"shadow"`; the wrong-for-3-weeks bug was fixed in commit 2). Producers can lean on this signal where they have domain context the policy doesn't.
- **The dispatch-via-Request-reissue pattern is reusable.** If a future commit adds a new resolve-flavor endpoint (e.g., bulk autonomous decide), it can follow the same pattern: load envelope, evaluate, reissue to the existing resolve route. No need to duplicate apply logic.
- **The `manualKinds` default is the Phase 6 safety belt.** It enforces "policy cannot auto-approve high-risk semantic Canon" by construction. Future consumers that opt out (set `manualKinds: []`) explicitly accept the responsibility for autonomous canon writes; the audit row records that choice via the policy version + reasons.