...
- `planning-scenes`: freeze the Phase-1 skeleton plus the same concept artifacts; measure scene-entry count per chapter, entry-kind mix, structural annotations, and whether the generated scene/turn list gives the mapper enough dramatic anchors. Harness plugs into `src/agents/planning-scenes/context.ts` + `src/agents/planning-scenes/schema.ts`.
- `planning-state-mapper`: freeze concept artifacts, chapter skeleton, and the fixed `planning-scenes` scene-entry list; measure `establishedFacts`, `knowledgeChanges`, `characterStateChanges`, `requiredPayoffs`, writer-visible obligation coverage, ignored mappings, retry count, and hard-failure rate after retry budget. Harness plugs into `src/agents/planning-state-mapper/context.ts` + `src/agents/planning-state-mapper/schema.ts` plus `src/harness/beat-obligations.ts`.
- `planning-state-repair`: freeze one invalid chapter outline plus its exact-ID validation packet; measure patch operation count, rejected operations, post-patch coverage, and fallback-to-mapper rate. Harness plugs into `src/agents/planning-state-repair/context.ts` + `src/agents/planning-state-repair/schema.ts` plus `applyBeatObligationRepairPatch()`.
...
- Entry-isolated eval for planner changes. Failure mode: a plan variant looks fine entry-by-entry but breaks payoff realization or chapter-level arc coherence. Missed interaction: `src/agents/planning-scenes/schema.ts` -> `src/agents/writer/beat-context.ts` -> `chapter-plan-checker`.
