...
- **halluc-leak-salvatore Rung 0 shipped (commit `cc57752`).** `checkHallucLeakSalvatore` now OR-combines a 59-token regex matcher with the W&B adapter at inference time. Production measurement on 3,081 calls: +31.6% recall, ≥95% precision on regex-only catches. Top adapter misses caught: Harpells, Baldur's Gate, Waterdeep. Zero SFT spend. Full report at `docs/rung-0-regex-ceiling-results.md`.
