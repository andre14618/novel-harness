...
- Diagnostic-only CLIs, disposable matrix runner, and read-only matrix list/viewer
  summarize/rank semantic gates, action/proposal evidence, plan-check drift
  witnesses, writer expansion, load-bearing checker evidence, and plan-assist
  lineage without changing runtime behavior.
