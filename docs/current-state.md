...
- **Beat-obligation shadow derivation is active (exp #284).** `src/harness/beat-obligations.ts` derives non-blocking per-beat obligations from existing planner output and logs orphan/overload warnings during planning. This does not yet change planner schema, writer prompts, checker prompts, or runtime severity. It measures whether `establishedFacts`, `knowledgeChanges`, and `characterStateChanges` are actually assigned to writer-visible beats before we promote an upstream contract into the Studio/writer/checker surfaces. Charter: `docs/charters/upstream-beat-obligations-v1.md`.
