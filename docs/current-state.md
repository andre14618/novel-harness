...
- **Cumulative prior-chapter context for the writer (L38).** Production `buildBeatContext` (`src/agents/writer/beat-context.ts`) emits per-beat surfaces (current beat spec, prior-beat transition bridge, character snapshots, setting) but does NOT aggregate establishedFacts from chapters 1..N-1 into the writer's prompt. An `enriched-context.ts` module that builds a `READER-INFO STATE` block aggregating prior-chapter facts and per-character `doesNotKnow` lists exists but is "DELIBERATELY not imported from any production code path" — it currently only serves the parity harness. L41-val novel `novel-1777721066908` ch2 bailed on 5 continuity blockers traced to this gap (writer dramatized fresh discovery for facts CH1 already established). Additional finding: `chapter_summaries` is empty for that novel (summarization step not running or not persisting). Diagnosis in `docs/l38-investigation-2026-05-02.md`. Sprint queued (todo §L38).
