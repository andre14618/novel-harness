...
- World fact role columns are additive on `facts` (`sql/049`) and
  ...
  `bun run eval:role-context-policy` covers deterministic fixture behavior; `bun run eval:fact-role-context-live-ab -- --source <id>` runs disposable legacy/role-aware clone comparison before production promotion, using resume-ready drafting clones.
