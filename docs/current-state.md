...
- **Phase-eval probe scaffold (Slice 0a + Slice 1, commits `a031980` + `c6ef9a5`).** Cheap-probe instrument from `docs/designs/phase-variant-comparison.md` charter (R5 — converged after 4 rounds of Codex `gpt-5.5 effort=high` adversarial review). Components:
  ...
  - `src/agents/planning-beats/index.ts` — `PLANNING_BEATS_PROMPT_OVERRIDE` env-var seam (read at module load, absolute path required). Used by per-variant child processes to inject a different beat-expansion prompt without rebuilding the agent registry. This seam now affects beat-shape expansion only; state/obligation mapping variants need a separate `PLANNING_STATE_MAPPER_PROMPT_OVERRIDE`-style seam before they can be probed cleanly.
  ...
  - `scripts/phase-eval/variants/planning-beats/{default,loud}.md` — pre-split variants from the old all-in-one planner. Treat their facts/knowledge/state-density gates as stale until the phase-eval harness is refreshed for the `planning-beats` + `planning-state-mapper` split.
...
- **Planner status — phase-eval probe instrument matured but needs split-mapper refresh.** `phase-variant-screen` branch carries the SCREEN-PASS verdict on the pre-split `loud` planning-beats variant against `fantasy-system-heretic` (Slice 1 end-to-end, retrospective at `docs/sessions/2026-04-29-phase-eval-probe.md`, Codex review integrated in `28c2e57`). Because `planning-beats` is now beat-shape only and `planning-state-mapper` owns state/obligation density, future phase-eval gates must choose which surface they vary. LXC probe of the corpus-v1 plotter variant came in mixed (Pattern 1 PASS — targetWords + beat-floor lift; Pattern 3a opener stable; Pattern 3b closer REGRESSED — 0/3 vs 2/3 action; Pattern 16 facts density dropped on n=3, unstable). Two follow-ups queued: revise the plotter's closer-kind guidance to fix the P3b regression before re-probing; re-measure P16 facts density on a 5-chapter sample.
