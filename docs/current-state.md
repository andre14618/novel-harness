...
- **Planner-authored beat obligations are writer-visible and coverage-validated (exp #286-#288).** `sceneBeatSchema.obligations` carries compact per-beat `mustEstablish`, `mustPayOff`, `mustTransferKnowledge`, `mustShowStateChange`, `mustNotReveal`, and `allowedNewEntities` lists. `planning-beats` emits them; `beat-context-render.ts` renders them as `BEAT OBLIGATIONS` in the beat-writer prompt. `src/harness/beat-obligations.ts` derives writer-visible coverage, and planning now triggers up to two targeted re-expansions for chapters with orphan facts/knowledge/state before approval/persistence. Optional obligation metadata and soft-prior arrays parse leniently as of `858e009`. Checker prompts/severity are not yet changed.
