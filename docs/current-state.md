...
- **Planner-authored beat obligations are writer-visible and coverage-validated (exp #286-#294).** `sceneBeatSchema.obligations` carries compact per-beat `mustEstablish`, `mustPayOff`, `mustTransferKnowledge`, `mustShowStateChange`, `mustNotReveal`, and `allowedNewEntities` lists. `planning-beats` now emits beat shape only; `planning-state-mapper` maps chapter-level facts/knowledge/state plus payoff links and beat obligations onto the existing beat list. `beat-context-render.ts` renders obligations as `BEAT OBLIGATIONS` in the beat-writer prompt. `src/harness/beat-obligations.ts` derives writer-visible coverage, planning triggers up to two targeted mapper retries for orphan facts/knowledge/state, and remaining gaps are auto-repaired into compact beat obligations before approval/persistence. The mapper maxTokens is 16384 after exp #292 showed 8192 could pass outline gates while still causing JSON/Zod/cap health failures. Retry prompts anchor existing state so the mapper fixes coverage by adding/moving obligations rather than deleting valid facts/state. **Coverage validation is exact-ID only (2026-05-01).** Every chapter, beat, character, fact, knowledge change, state change, payoff, and obligation receives a stable kebab-case ID via `enrichOutlineIds()` in `src/harness/ids.ts` (idempotent; runs after every mapper merge and after auto-repair). The validator builds source registries keyed by ID; a coverage gate passes only when each source ID is referenced by at least one obligation's `sourceId`. Fuzzy beat-text overlap, character-name aliasing, and substring text matching are now diagnostic-only (`summary.implicitTextMatches`) — they never contribute to a passing gate. Auto-repair builds replacement obligations with explicit `obligationId`/`sourceId`/`sourceKind`/`characterId`. The state-mapper prompt instructs the model to emit those fields directly; the legacy `id`/`factId`/`characterName` fields remain accepted for round-trip. Retry feedback names missing source IDs explicitly. `scripts/phase-eval/print-screen-verdict.ts` G1 for the state-mapper metric set is now `missing_source_ids = 0 AND unknown_source_ids = 0 AND duplicate_source_ids = 0 AND character_id_mismatches = 0`. Beat-writer `llm_calls.request_json` carries `chapterId`, `beatId`, `obligationIds`, `sourceIds`, `characterIds` for prose-to-plan provenance lookup; the writer prompt bytes are unchanged (preserves the `tests/beat-context-parity.test.ts` byte-parity gate). Checker prompts/severity are not yet changed.
