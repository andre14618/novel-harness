...
Lane monitoring is repo-local and tool-neutral. `scripts/agent/lane-heartbeat.ts` appends outside-loop events to `output/agent-runs/<lane-id>/events.jsonl`; `scripts/agent/lane-status.ts` computes `continue|stop|blocked|human-needed|infra-failure`; `monitor` / `scripts/agent/lane-dashboard.ts --watch` render the outside loop, inside-harness summary, evidence rows, repo hygiene, and process health through `--panel all|outside|inside|evidence|hygiene|process`. `scripts/agent/lane-runner.ts` can launch bounded unattended worker cycles while lane-status remains `continue`: `opencode run` by default or `claude -p` with `--engine claude`. It records cycle artifacts under `output/agent-runs/<lane-id>/cycles/`, stops on status changes, worker failures, max cycles/hours, or no tracked workspace change, and can advance through a queue of pre-created lane docs only after the stopped lane's Results fields are complete. Inside-harness novel data delegates to `scripts/operator-summary.ts`. Abandoned pending plan-assist gates should be marked `decision='orphaned'` with `scripts/agent/resolve-stale-gates.ts` after dry-run review, never deleted. See `docs/agent-lane-protocol.md`.
...
- **Cumulative prior-chapter context for the writer (L38).** Production `buildBeatContext` (`src/agents/writer/beat-context.ts`) emits per-beat surfaces (current beat spec, prior-beat transition bridge, character snapshots, setting). L38-A (2026-05-02, exp #369) wired the READER-INFO STATE block into the production beat context for chapters > 1: prior-chapter establishedFacts and per-present-character `doesNotKnow` now render as a section between resolved-references and SETTING (mirroring `insertEnrichedSection`'s anchor in the Arm B preflight). Slot-selection lives in `selectReaderInfoStateForBeat` (`src/agents/writer/enriched-context.ts`) and is gated to chapter > 1. Drafting wires `getFactsUpToChapter(novelId, ch - 1)` at all three `buildBeatContext` call sites (`src/phases/drafting.ts`). Byte parity for chapter-1 fixtures is preserved (fixtures pass no `priorChapterFacts` and the gate returns null). Open: heretic ch2 post-deploy smoke (lane stop gate L38-A). Remaining L38 sub-lanes: planner-side prior-fact context (L38-B), `chapter_summaries` wiring (L38-C), continuity calibration (L38-E). Diagnosis in `docs/l38-investigation-2026-05-02.md`; lane in `docs/sessions/2026-05-02-L38-A-prior-context.md`.
