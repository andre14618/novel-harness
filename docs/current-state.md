...
Bare `monitor` is the compact default alias for human loop visibility: it shows `outside`, `coordination`, and `process` panels and hides latest-novel wall text. Use `monitor --full` for all panels plus the latest novel summary, or `monitor --append` only when repeated snapshots are intentional. Captured/non-TTY watch mode renders one snapshot instead of flooding output.
