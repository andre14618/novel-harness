...
- **Planner-authored beat obligations are writer-visible (exp #286).** `sceneBeatSchema.obligations` carries compact per-beat `mustEstablish`, `mustPayOff`, `mustTransferKnowledge`, `mustShowStateChange`, `mustNotReveal`, and `allowedNewEntities` lists. `planning-beats` emits them; `beat-context-render.ts` renders them as `BEAT OBLIGATIONS` in the beat-writer prompt. `src/harness/beat-obligations.ts` still derives shadow obligations and logs orphan/overload telemetry during planning. Hardened commit `1f62210` parses optional obligation metadata leniently. Fresh run `novel-1777597799926` assigned all facts but still left knowledge/state orphans in chapters 1-2, so checker prompts/severity are not yet changed.
