...
- **Planner-authored beat obligations are writer-visible and coverage-validated (exp #286-#288).** `sceneBeatSchema.obligations` carries compact per-beat `mustEstablish`, `mustPayOff`, `mustTransferKnowledge`, `mustShowStateChange`, `mustNotReveal`, and `allowedNewEntities` lists. `planning-beats` emits them; `beat-context-render.ts` renders them as `BEAT OBLIGATIONS` in the beat-writer prompt. `src/harness/beat-obligations.ts` derives writer-visible coverage, planning triggers up to two targeted re-expansions for orphan facts/knowledge/state, and remaining gaps are auto-repaired into compact beat obligations before approval/persistence. Validation run `novel-1777601516385` on `8d57662` reached zero obligation orphans after retries. Optional obligation metadata and soft-prior arrays parse leniently as of `858e009`. Checker prompts/severity are not yet changed.
...
- `src/harness/beat-obligations.ts` — shadow assignment from planner state to per-beat obligations
