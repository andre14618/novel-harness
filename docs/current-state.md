...
- **Planner-authored beat obligations are writer-visible (exp #286/#287).** `sceneBeatSchema.obligations` carries compact per-beat `mustEstablish`, `mustPayOff`, `mustTransferKnowledge`, `mustShowStateChange`, `mustNotReveal`, and `allowedNewEntities` lists. `planning-beats` emits them; `beat-context-render.ts` renders them as `BEAT OBLIGATIONS` in the beat-writer prompt. `src/harness/beat-obligations.ts` still derives shadow obligations and logs orphan/overload telemetry during planning. Optional obligation metadata and soft-prior arrays parse leniently as of `858e009`. Fresh runs assign all facts but still show variable knowledge/state orphan coverage, so checker prompts/severity are not yet changed.
