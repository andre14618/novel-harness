...
- **lint-fix integrity guard** — after deterministic/LLM lint fixes run, `src/lint/integrity.ts` rejects malformed post-fix prose before `saveChapterDraft()` overwrites the raw draft. The guard currently blocks newly introduced fused sentence boundaries, dropped-space camel fusions, and adjacent duplicate sentences; failures emit `lint-fix-rejected` trace events and keep the raw draft.
