...
Lane monitoring is repo-local and tool-neutral. `scripts/agent/lane-heartbeat.ts` appends outside-loop events to `output/agent-runs/<lane-id>/events.jsonl`; `scripts/agent/lane-message.ts` appends addressed operational requests, claims, handoffs, and results to `output/agent-runs/<lane-id>/messages.jsonl`; `scripts/agent/lane-status.ts` computes `continue|stop|blocked|human-needed|infra-failure`; `monitor` / `scripts/agent/lane-dashboard.ts --watch` render the outside loop, operational coordination, inside-harness summary, evidence rows, repo hygiene, and process health through `--panel all|outside|coordination|inside|evidence|hygiene|process`. The coordination panel shows the active runner worker id, latest heartbeat actor, claimed work by actor, messages, and expired leases. `scripts/agent/lane-runner.ts` can launch bounded unattended worker cycles while lane-status remains `continue`: `opencode run` by default or `claude -p` with `--engine claude`. Use `--worker-role` and `--worker-id` to make the durable actor explicit, such as `captain-claude` or `evidence-dsv4`; the worker prompt tells the agent to use that id for heartbeats and messages. For supervised work, `--worker-io terminal` launches visible Claude Code without `-p` or OpenCode TUI with `--prompt`, while still using lane heartbeats/messages as durable state. It records cycle artifacts under `output/agent-runs/<lane-id>/cycles/`, stops on status changes, worker failures, max cycles/hours, or no tracked workspace change, and can advance through `docs/sessions/lane-queue.md` only after the stopped lane's Results outcome, stop gate, evidence, and commits are complete. Documentation closeout can be handed to `.opencode/agent/docs-finalizer.md` through `scripts/agent/finalize-docs.ts`, which first writes a deterministic `finalizer-packet` under `output/agent-runs/<lane-id>/`, then invokes OpenCode on `deepseek/deepseek-v4-flash` with a high reasoning variant and authorizes a docs-only commit after docs-impact and whitespace checks pass. The worker prompt includes end-of-lane finalization: docs sweep, experiment conclusion, stale-gate orphaning, docs-impact/whitespace checks, and a final cleanup commit before handoff. Inside-harness novel data delegates to `scripts/operator-summary.ts`. Abandoned pending plan-assist gates should be marked `decision='orphaned'` with `scripts/agent/resolve-stale-gates.ts` after dry-run review, never deleted. See `docs/agent-lane-protocol.md`.
