...
- For fantasy seeds, Track A validation routes the Salvatore-shaped fantasy pack to base DeepSeek V4 Flash (non-thinking), not the W&B Salvatore voice LoRA. The pack still carries the Salvatore-derived system prompt, compact context path, structural priors, and Salvatore leak-check gating.
