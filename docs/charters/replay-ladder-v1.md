...
|----------|---------|------|-------|
| successor charter (2026-04-21) | `arm-b-direct-pairwise` | 2026-04-21 | After the preflight went 9 review rounds and a meta-consult (job `a738b4bb2879c39d0`) found detector-precision-preservation to be a weak proxy for "does enriched context improve prose," the preflight was superseded by `docs/charters/arm-b-direct-pairwise.md` — blind human pairwise on 20 matched beats with detector fire-rate delta as secondary telemetry. Verdict from the direct instrument gates this ladder's Arm B. |
| preflight outcome (2026-04-21) | INCONCLUSIVE | 2026-04-21 | Exp #260. Arm-B detector preflight (child charter, GREEN round 6) executed end-to-end on `novel-1776690960321`: 10 beats, 2 fires A / 3 fires B, $0.0023 writer cost, 0 errors. Raw fire counts below charter §3 adjudicable-fires floor — verdict locked to INCONCLUSIVE regardless of adjudication. **Charter design flaw:** preflight revision 2 rebased to V1 28.9% fire rate but kept the 8-fire floor + 20-beat cap from the V0 44.9% era; mathematically infeasible at any sample under ~28 beats. Preflight infrastructure proven sound; two live-execution-only bugs fixed (commit `0ff8646` — request_json-as-TEXT + JSONB-as-string). See `docs/charters/arm-b-detector-preflight-results.md` for full writeup and three successor-charter options (widen cap to ~40, higher-fire stratum, combined-detector oracle). Ladder run remains blocked pending successor charter. |
