...
- **Adherence-checker — PROMPT SWAP NEXT, SFT DEMOTED to conditional.** Exp #122 closed half the SFT gap with no training: 4-call decomposed prompt lifts 14B 79% → 91% and 235B 96% → 97%. Ship the prompt swap first (`src/agents/writer/adherence-checker.ts`), validate against a real novel run, THEN re-decide whether the remaining 6pp gap on 14B is worth a training run. If yes, label through the *decomposed* 235B (97%) — not the flat single-call prompt — so the student matches production shape.
- **Chapter-plan-checker** — already validated, see `project_chapter_plan_checker_finetune.md`. SFT distillation from gpt-oss-120b on the existing 80 synthetic pairs + accumulated production pairs. One-sided bias makes this a textbook SFT case. **Per-beat decomposition was tested and disconfirmed** (#123: gpt-oss 90% → 64%, 235B 81% → 72%) — flat single-call stays as the production shape, distillation is the path.
