...
<!-- Latest: 2026-05-01 — labeled current-surface calibration (#301) + synthetic invocation (#302) + halluc-ungrounded v3 prompt promotion (#303). v1 → v3 lifted recall from 10% to 60-70% on the labeled panel (F1 0.18 → 0.63-0.74); v3 deployed to LXC. Remaining: temp=0 stability check, then obligation-aware adherence checks. See decisions.md. -->
...
- [ ] **Design obligation-aware beat checks for adherence-events first.** adherence-events is calibrated for stricter contract (100% precision / 100% recall on natural + synthetic). halluc-ungrounded now at ~65% recall — usable but worth one more pass on stability before pairing it into a stricter combined gate.
