...
1. **Swap continuity 235B to the checklist prompt in production.** Exp #118 showed +0.086 F1 on the highest-cost-per-call agent in the pipeline. Latency 389ms → 638ms (~1.6×), output tokens 59 → 380 (~6×), well within per-chapter budget. Do this BEFORE any SFT spend. Source: `scripts/score-continuity-checklist.ts` CHECKLIST_SYSTEM constant. Wire to `src/agents/continuity/prompt.md` and validate against a real novel.
