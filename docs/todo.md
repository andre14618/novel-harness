...
- [ ] **Successor to `arm-b-detector-preflight` — charter one of three options** (exp #260 INCONCLUSIVE per charter §3 math error: 8-fire floor + 20-beat cap infeasible at V1 28.9% production fire rate). Paths, ordered by recommendation in `docs/charters/arm-b-detector-preflight-results.md`: (1) **revision 7 widens the beat cap** to ~40 (cleanest, simplest, needs one Codex round), (2) higher-fire stratum via chapter-start / lore-density predicate, (3) new charter for combined-detector oracle (adherence + ungrounded + leak) measuring aggregate beat-check precision instead of halluc-ungrounded specifically. Parent charter `replay-ladder-v1.md` §10 updated to reflect this; ladder run remains blocked pending successor.
