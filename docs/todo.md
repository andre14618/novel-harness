...
- [ ] **Re-record phase-parity fixture on LXC after merge.** Pre-merge Codex review (`a8bb6556c14c3f098`, gpt-5.5) flagged that `tests/phase-parity/replay-transport.ts` was hashing requests without `extraBody`, so the parity test would silently match thinking-on requests against thinking-off recorded responses. Fix landed: `hashRequest()` now includes a canonicalized `extraBody`, and `RecordedCall.request.extraBody` captures the field on record. Existing fixture at `tests/phase-parity/fixtures/reference-run/transport-fixture.json` was recorded before the capture landed, so its stored requests have no `extraBody` — replays for any thinking-on agent (planning-beats / chapter-plan-checker / chapter-plan-reviser / structure-promise / structure-character-arcs) will correctly miss until the fixture is re-recorded against the current agent config. Re-record via `ssh novel-harness-lxc "cd ~/apps/novel-harness && bun tests/phase-parity/record-fixture.ts fantasy-system-heretic"` once the merged code is deployed. Not blocking: parity test skips locally (no DATABASE_URL); the loud-failure-on-LXC behavior is the correct safety-net response.
...
- [ ] **Voice-shaping bundle as the LoRA-replacement test (HIGH PRIORITY).** Composite prior bundling P29 + P39 + P57 + P65 + P67 into a writer-prompt variant under `WRITER_GENRE_PACKS` fantasy-Salvatore. Three-arm probe: (A) DeepSeek V4 Flash + bundle prompt, (B) Salvatore voice LoRA v3 (current prod), (C) DeepSeek V4 Flash baseline (no bundle). Verdict shape: PASS → bundle matches/beats LoRA → candidate to retire LoRA for fantasy route; PASS_PARTIAL → bundle becomes route for non-fantasy genres without a voice LoRA; KILL → voice transfers via weights only (re-evaluate). Charter required (`/charter-review` SOP); measurement scaffold (brief-set + variant runner + deterministic+Sonnet-judge scorers) is the precondition. Driving from `synthesis-bundle-v1` branch starting 2026-04-30.
