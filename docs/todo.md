...
- [ ] **Investigate halluc-ungrounded temp=0 stability.** v3 prompt at temp=0.1 showed run-to-run variance (60% vs 70% recall on the same panel). Consider lowering to temp=0 for the checker call, or expanding the prompt with more worked examples to anchor the title+surname / institution rules deterministically. The Master Orin natural FN was consistent across both runs — a separate gap not closed by lowering temp.
