...
2. **Submit training job via ART + W&B Serverless SFT** — format the 111 reviewed pairs as ART-compatible JSONL, configure ART with `base = OpenPipe/Qwen3-14B-Instruct`, `rank = 16`, `alpha = 16`, `epochs = 1–2`, `ServerlessBackend`. ART auto-saves the checkpoint as a W&B artifact and makes it immediately routable. No GPU provisioning or manual upload needed. Docs: https://art.openpipe.ai/fundamentals/sft-training
