...
- [ ] **halluc-leak-salvatore training-data gap on canonical FR names** — the beat-entity-list V1 run (exp #254) exposed partial recall on the leak adapter. Real leak fire rates: V0 7.1%, V1 6.3% (the adapter schema is `{"has_leak":true,"leaks":[...]}`, not `pass:false`; the "0 fires" claim in the initial summary was a query-filter bug). Per-entity recall: caught Ten-Towns 1/2, Luskan 1/2, Do'Urden 1/2, Maer Dualdon 1/2, Bryn Shander 1/1; missed Waterdeep 4/4 and Baldur's Gate 3/4 consistently. Halluc-ungrounded caught all misses as the corpus-agnostic safety net (designed behavior). Action: add Waterdeep + Baldur's Gate + the inconsistent FR names to `halluc-leak-salvatore` positive training examples for v2; seed from these 16 V0+V1 fires as active-learning candidates.
