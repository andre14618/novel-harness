...
- [ ] **halluc-leak-salvatore under-firing** — on the beat-entity-list V1 run the leak adapter fired 0 times despite V1 ungrounded catching 7+ Salvatore-corpus names (Waterdeep, Luskan, Ten-Towns, Bryn Shander, Do'Urden, Baldur's Gate, Drossen Ironbelly). Separate from the v3 work — likely adapter-side under-recall, not retry-wording. Needs isolated eval before any retrain decision.
