...
- [ ] **[high] Together fine-tune submitter reuses stale uploads by basename match.** `scripts/finetune/train-together-small.py:39-74` selects prior uploaded Together files by filename. If a local JSONL was regenerated, training can silently use stale content. Script advertises `--experiment-id` but never persists it, so there's no provenance trail. Fix: select by content hash OR always upload fresh, log resolved file_id, persist `job_id`/`file_id`/data SHA when `--experiment-id` supplied.
- [ ] **[high] Leak formatter throws away Sonnet label corrections.** `scripts/hallucination/format-v3-two-adapters.ts:156-180`: `applyLabel()` applies Sonnet's corrected verdict, but `toLeakPair()` ignores it and recomputes `has_leak` only from `_meta.variant`. A PASS example that Sonnet flipped becomes a negative training row. Poisons the leak adapter training. Fix: derive `has_leak` + `leaks[]` from the corrected assistant payload or Sonnet result, not from the original variant. Rebuild any leak training/eval artifacts derived from the current formatter after the fix.
- [ ] **[medium] Natural leak val auto-labels generic names as positives.** `scripts/hallucination/build-natural-leak-val.ts:17-47` substring-matches against `LEAK_TERMS`, which includes generic one-token names (`Cassius`) that are ALSO in the benign injection pool. Clean prose gets marked leak-positive; detector training reward for overfiring. Fix: remove ambiguous one-token names from auto-labeling OR hand-label this eval set OR use phrase-level disambiguation.
- [ ] **[medium] `qualityRedraftEnabled` flag is process-wide, not per-run.** `src/config/pipeline.ts:20-24` reads `QUALITY_REDRAFT_ENABLED` at module load from the host env. If set at the orchestrator-service level (systemd unit or exported), every subsequent novel inherits the flag until process restart. Today's measurement novel (PID 315593) is safely scoped because launched via one-shot `QUALITY_REDRAFT_ENABLED=true nohup bun src/index.ts`, but the design issue remains for orchestrator-path usage. Fix: accept a `--quality-redraft` CLI flag OR thread through `run_config` instead of env var at module load.
...
- [ ] **halluc-leak-salvatore regex FN follow-up.** 12 residual beats where adapter caught and regex missed: "dark elf" (generic, not in token list), "Rumblebelly's" (possessive form), "mithril" (lowercase standalone). Three genuine regex FNs (verbeeg/Aegis-fang inconsistencies). Cheap widen pass: add possessive-suffix tolerance to regex, add "dark elf" variants, re-measure. ~1h work, $0.
