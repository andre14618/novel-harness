...
- [~] **Track A: validate-and-retire the Salvatore writer LoRA (HIGH PRIORITY).** Voice-shaping-ablation-v1 already found bare DeepSeek V3.2 near the Salvatore reference distribution on 3/5 measurable voice-shape axes, while Salvatore v4 leaked corpus tokens and had extreme word-count outliers. First move is therefore operational validation, not a v2 charter: route `WRITER_GENRE_PACKS` fantasy-Salvatore to base DeepSeek V4 Flash, deploy, run a 3-chapter `fantasy-system-heretic` validation novel, and read prose. If quality holds, record the decision and close the writer-LoRA production route. If a concrete gap appears, reopen the measurement scaffold / v2 bundle work only around that gap (likely dialogue ratio, sensory density, or character distinctness).
