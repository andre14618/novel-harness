...
- [x] ~~Evaluate stable IDs for knowledge/state changes.~~ **Done 2026-05-01.** Stable IDs now flow through every chapter-level item and every beat obligation; coverage validation is exact-`sourceId` only and fuzzy text matching is diagnostic-only. See decision "Stable-ID deterministic contract" and `src/harness/ids.ts`.
- [ ] **Soak the stable-ID coverage gate on a real mapper screen.** Run `coverage-balanced` vs `default` on `fantasy-system-heretic` under the new exact-ID G1 (`missing_source_ids = unknown_source_ids = duplicate_source_ids = character_id_mismatches = 0`). Compare `implicit_text_matches` (diagnostic) — high values flag obligations the LLM should have authored but didn't, and may motivate prompt tightening. Persist findings under a new tuning_experiment.
