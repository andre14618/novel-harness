...
When a harness operation issues more than one DB statement (UPDATE proposals + UPDATE supersession + INSERT canon + INSERT generation counter), the work must run inside a single `db.begin(async (tx) => { … })` block — otherwise a crash between statements leaves the DB inconsistent in ways that violate the substrate's documented invariants. The cheap implementation pattern: every DB-helper function takes an optional last parameter `executor: SQL = db` and uses it instead of importing `db` directly. When called outside a transaction, the default works. Inside a `db.begin` block, the harness threads `tx` through. No global state, no thread-local context, no ORM-level abstraction required — just a parameter. Caught by Codex round-2 review of `ba72e09` (HIGH finding: proposal resolution + canon commit + generation bump were three separate round-trips); the fix is mechanical once the executor-param pattern is in place. The pattern is documented in the canon-substrate `src/db/canon-substrate.ts` header for future module authors.

### Schema-level invariants > application-level invariants when the schema IS the contract (2026-05-03)

When a substrate's design doc says "at most one active version per logical id," the right enforcement layer is the schema (`CREATE UNIQUE INDEX … WHERE superseded_by_version IS NULL`), not application code. Application-level enforcement breaks the moment a future caller — operator UI, batch importer, CLI tool, ad-hoc `psql` session — bypasses the harness. The partial unique index is cheap (one CREATE INDEX per table) and load-bearing: if app code tries to write a row that violates the invariant, the constraint fires and surfaces the bug instead of silently corrupting reads. Same logic applies to value-range checks: `Provenance.confidence` documented as `[0, 1]` but typed `NUMERIC(4,3)` permits 9.999; a CHECK constraint catches the bug at the boundary. Codex round-2 caught both; the fix is a separate migration that goes alongside the schema. Rule of thumb: if a documented invariant is stated as "the substrate guarantees …", it has to be a schema constraint, not just a comment in the harness module. (`sql/036_canon_substrate_invariants.sql`.)
...
### Deterministic id is the cheapest idempotency primitive when the DB has a unique key (2026-05-03)

When a service must be idempotent against retries — same input on a second run produces 0 new rows — the cheapest implementation is to make the id a deterministic function of the input (`<source>:<namespace>:<input-id>:<schema-version>`) and use `INSERT … ON CONFLICT (id) DO NOTHING RETURNING id` for the write. The boolean returned (rows.length > 0) tells the caller whether it inserted; `created` and `skipped` lists fall out of that. Two properties this gets you for free: (1) operator state survives reruns — a rejected/approved row is keyed by the same id the rerun would insert, and ON CONFLICT refuses to overwrite, so the operator's "no" / "yes" is durable; (2) two namespaces (e.g., two novels) with overlapping inner ids stay disjoint because the namespace is part of the id. The alternative — separate "has this been processed?" lookup followed by INSERT — is racy and slow. The schemaVersion suffix is the only tunable: bump it when the payload shape changes and reruns produce fresh rows alongside historical ones; operators see both, and the historical resolution stays authoritative for the prior version. Pattern shipped in `src/harness/planner-canon-proposals.ts` Phase 1 (2026-05-03), proven by 30-row idempotency test (first run: 30 created / 0 skipped; second run: 0 / 30; DB row count stays exactly 30).

### Split DB-writing services into a pure mapper + a thin DB-writing wrapper (2026-05-03)

When a harness service is a thin layer over a DB write (gate-check + payload-shape + INSERT), split it into two functions: a **pure** `buildXxx(...)` that returns the proposal/payload list with no DB calls, and a **wrapper** `generateXxx(...)` that wraps the pure helper and does the writes. This solves three problems at once: (1) tests for the mapping logic don't need a DB, so the InMemory tier is a real signal — gate fail-closed, per-kind FactKind/ProvenanceSource, deterministic-id format — even when Postgres is unreachable; (2) review-preview / dry-run UIs (Phase 2 of the proposal workflow, in this case) get a real surface to call without persisting; (3) the wrapper stays tiny and obviously-correct. The split costs ~5 lines and removes the temptation to write a fake-DB harness for unit tests. Pattern: `buildPlannerCanonProposals` (pure) + `generatePlannerCanonProposals` (wrapper), `src/harness/planner-canon-proposals.ts`.
...
### Calibration panels must be labeled per-checker, not pooled (2026-05-01)
Exp #301 labeled the same 17 beat attempts twice — once for `halluc-ungrounded` (judged against `groundedSources` allowlist) and once for `adherence-events` (judged against `beat_description` enactment). The two checkers had wildly different calibration profiles on the SAME prose: halluc 12.5% recall / 50% precision vs adherence 100% / 100%. Rule: never pool checkers when computing calibration matrices. Each checker has its own evidence surface, its own oracle rubric, and its own failure modes. A "panel pass rate" averaged across checkers is meaningless — and worse, it would have hidden the halluc severity-promotion blocker behind the adherence calibration. The labeling task itself can run in parallel (different subagents per checker) but the matrices must be reported separately.

### Subagent batching for calibration labeling (2026-05-01)
Exp #301 used 4 parallel Sonnet subagents (2 per checker × 8-9 rows each) to label 34 natural rows. Each subagent received: (a) rubric file path inline, (b) a JSON file with their batch slice, (c) explicit output schema and write-to-file path. Wall-clock 2 minutes vs full-day manual labeling. Two engineering details made it work: (i) extract MINIMAL labeling payload — for halluc, the per-row data is `prose + grounded_sources + checker_actual_pass + checker_actual_issues`; nothing else; (ii) require quote evidence in the output schema so the labels are auditable. Failure mode caught at aggregation: one subagent dropped the `-halluc-ungrounded` suffix from `fixture_id`, which caused naive string-matching aggregation to mis-tag rows. Rule: when subagents return into a structured aggregation, normalize fixture identifiers explicitly (suffix-aware) rather than substring-matching the checker name into the ID.

### Variant-override prompts must be re-synced when the live system prompt changes (2026-05-01)
Exp #295's first run failed because `scripts/phase-eval/variants/planning-state-mapper/{default,coverage-balanced}.md` were silently stale relative to the live `src/agents/planning-state-mapper/state-mapper-system.md` after the stable-ID sweep. A `*_PROMPT_OVERRIDE` env var completely replaces the live system prompt — it does not patch or overlay it — so any non-trivial change to the live prompt's contract (in this case, the new `id`/`characterId`/`obligationId`/`sourceId`/`sourceKind` fields and the exact-ID coverage rules) instantly diverges every variant override file from the runtime contract. Rule: when changing a system prompt that has variant overrides under `scripts/phase-eval/variants/`, re-sync every override in the same commit and `diff` the "default" variant against the live prompt as a parity check. The probe gives no warning when the override drifts; the only signal is downstream validator failure on every output. The screen failure is the trace guard doing its job, but the cheaper debug path is to grep variant dirs at change time.
...
Exp #293 exposed a deterministic repair mismatch: code-authored repair could add a short exact `mustTransferKnowledge` item, but validation still failed because the matcher required at least two meaningful-token overlaps. Rule: stable-ID contract validation should use explicit `sourceId`/`sourceKind`/`characterId` equality only. Text overlap should not participate in validation or deterministic repair.
...
W&B Inference latency probe (`tuning_experiment` id=94, `scripts/archive/finetune/test-wandb-inference.ts`) hit a counterintuitive finding: **OpenPipe/Qwen3-14B-Instruct on W&B Inference is FASTER than Cerebras Qwen 235B on the adherence-checker workload shape** — 157ms avg vs 365ms avg, a 2.3× speedup. On a 14B model on a "standard" tier inference service vs a 235B model on Cerebras's custom LPU hardware. That's not the direction the comparison usually goes.
...
## Fuzzy text matching does not belong in stable-ID contract paths
...
**The rule:** stable-ID contract paths should not use fuzzy matching for either validation or deterministic linking. A contract gate that accepts paraphrase + token-overlap as a passing signal will degrade silently as either the prompt or the matching heuristic evolves; the only way to keep the gate meaningful is to require an explicit reference (`sourceId === id`) that downstream consumers can also rely on.

**How to apply:**
- Validation gates check exact equality on stable IDs. Enrichment assigns artifact IDs and obligation IDs, but must not infer obligation `sourceId` from text.
