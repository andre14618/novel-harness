...
### Penalty counts don't capture writer prompt adherence (2026-04-06)
Compared Qwen 235B ($0.60/$1.20), DeepSeek V3.2 ($0.28/$0.42), and MiMo Flash ($0.10/$0.30) on 3 seeds × 2 runs. Penalty scores overlapped within variance, suggesting equivalence — but reading the actual prose reveals differences the benchmark misses. MiMo Flash produces more exposition monologues, theme-stating ("Control. That was the word"), and characters who narrate plot mechanics instead of acting. It also leaked OCR metadata in one call, suggesting schema adherence issues under edge conditions. Penalty counts measure generic tells; they don't measure adherence to the 73+ craft rules in the writer prompt (character voice consistency, subtext in dialogue, show-don't-tell discipline). **Don't conclude model equivalence from penalty counts alone — read the prose.** MiMo Flash is viable for fast iteration but not for final drafts. (Experiment #90)
