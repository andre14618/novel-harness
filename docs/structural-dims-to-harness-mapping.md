...
- **Injection site**: `src/agents/planning-scenes/scene-expansion-system.md` (the Phase-2 per-chapter scene-entry expansion system prompt). The entry's `kind` field already exists in `sceneBeatSchema`; the MICE label is a parallel annotation.

- **Concrete prompt edit**: Add a new optional field `mice_thread` to each scene entry in the system prompt's JSON example, with a soft-prior instruction block (not a hard-schema requirement yet):

  ```
  ## MICE thread annotation (soft prior — include when unambiguous)
  Each entry may carry a "mice_thread" field: { "primary": "M|I|C|E", "opens": true|false, "closes": true|false }
  ...
  Omit "mice_thread" when the entry's primary function is transitional or multi-threaded.
...
- **Data shape**: Per-entry tag (primary: M|I|C|E, opens: bool, closes: bool). The chapter-level distribution of opens/closes is the constraint — not per-entry enforcement. A chapter should contain MICE thread opens that are closed within 2-5 entries (Sanderson's satisfying-loops heuristic).

- **Constraint type**: **Soft prior** — included in the system prompt as a target distribution and annotated per-entry, but no hard schema validation gate. Do not gate chapter approval on MICE compliance until C.4 sharpening brings P ≥ 0.78.

- **Implementation note**: The `planning-scenes/schema.ts` imports from `../../schemas/shared`, so the addition to `sceneBeatSchema` propagates automatically. The beat-writer context (`src/agents/writer/beat-context.ts`) passes entry descriptions to the writer but not `kind` annotations — `mice_thread` would remain a planner-only field and not flow to the writer, which is correct (MICE is a structural constraint on the plan, not a prose instruction).
...
- **Injection site**: `src/agents/planning-scenes/scene-expansion-system.md` — same file as mice, appended as a separate section.

- **Concrete prompt edit** (to add once retest confirms CELL PASS):

  ```
  ## Value-charge per entry (soft prior)
  Each entry optionally carries a "value_charge" field:
...
- **Data shape**: Per-entry struct (life_value label + start/end polarity). The constraint is distributional: a chapter's entry sequence should alternate polarity, not sustain one charge for more than 4 entries.

- **Constraint type**: **Soft prior now, upgrade to hard after n ≥ 20 retest.** The polarity-shift cadence is a planning-scenes instruction only; the writer doesn't need to see the schema field.

- **Implementation note**: The life_value taxonomy is open-text — the planner picks the value, not the harness. The scene-expansion prompt should give 4-6 canonical examples drawn from the Crystal Shard data (`value-charge.20260429T183957.jsonl`) so the planner anchors to recognizable categories: life/death, honor/dishonor, freedom/slavery, love/hatred, loyalty/betrayal.
...
- **Injection site (anticipated)**: `src/agents/planning-scenes/scene-expansion-system.md`. McKee-gap is per-entry (expectation vs. outcome divergence), making scene-entry expansion the natural site — same as mice and value-charge.

- **Concrete prompt edit (draft — hold until calibration completes)**:

  ```
  ## McKee gap per entry (structural tension)
  Each entry optionally: "mckee_gap": { "gap_size": "none|small|large", "gap_type": "yes_but|no_and|yes" }
  An entry with gap_size "large" means the POV character's expectation was sharply violated — the scene outcome diverged from their plan. Crystal Shard target: >60% of entries carry a gap; pure "yes" entries (expectation met, no gap) should not run more than 2 consecutive.
  ```

- **Data shape**: Per-entry struct (gap_size enum + gap_type enum). Expectation is defined by the POV character at entry start; outcome is what actually happens.
...
- **Injection site (when ready)**: Two sites — `src/agents/plotter/story-structure-system.md` (for arc-level promise tracking) and `src/agents/planning-scenes/scene-expansion-system.md` (for Chekhov's-gun setup-payoff bridges within chapters). The per-entry `requiredPayoffs` field (already in `sceneBeatSchema`) is structurally the right shape for intra-chapter promises. Cross-chapter promises need a new field in the chapter skeleton or the planner context.

- **Concrete prompt edit (draft — do not ship yet)**: The `requiredPayoffs` mechanism already in planning-scenes (fact_id + payoff_beat) is the right sub-schema for intra-chapter setup-payoffs. For cross-chapter arc-level promises, the plotter's `story-structure-system.md` would add a `"promisedPayoffs"` field on each act: a list of `{ promise: string, opens_at_chapter: int, closes_by_chapter: int }`. The `planning-plotter/chapter-outline-system.md` would then receive that act's promises as constraints when generating its chapter skeletons.

- **Data shape**: Two levels — (a) intra-chapter: existing `requiredPayoffs` on entries covers this; (b) cross-chapter: arc-level list of open/close tuples, one per named promise. Crystal Shard's Sonnet-extracted list (`promises.20260429T215322.sonnet.json`, 38 promises) is the reference shape.
...
**Where one constraint subsumes another**: The `requiredPayoffs` field in `sceneBeatSchema` already handles intra-chapter setup-payoffs (a subset of what promise tracks at book scale). Do not add a parallel promise mechanism at the entry level — use `requiredPayoffs` for that scope.
...
2. **mice** (after C.4 sharpening) — add `miceThread` optional field to `sceneBeatSchema` in `src/schemas/shared.ts` + add soft-prior block to `planning-scenes/scene-expansion-system.md`.
3. **value-charge** (after n ≥ 20 retest confirms CELL PASS) — add `valueCharge` optional field to `sceneBeatSchema` + add soft-prior block to `planning-scenes/scene-expansion-system.md`.
...
- **Mice chapter-level target vs. per-entry field**: The calibrated distribution (I ~38%, E ~28%, C ~22%, M ~12%) is across scenes, not chapters. A per-chapter MICE distribution constraint in the planning-scenes prompt is achievable but untested — start with per-entry annotation and aggregate the distribution post-hoc before adding per-chapter targets.
