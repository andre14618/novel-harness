...
*2026-04-07 · exp #94 (`scripts/archive/finetune/test-wandb-inference.ts`)*
...
### L74 — Phase 6 review fixes + Phase 7 replay-metrics tracer bullet (2026-05-04)

**Decision:** Before opening new runtime behavior, close the focused Phase 6 review findings and start Phase 7 with a read-only replay report surface. Phase 6 fixes are: canon `bulk-resolve` now threads `resolvedByKind` and `resolution_policy_*`; canon single resolve runtime-validates `resolvedBy`; policy-decide queue decisions re-check pending status under `SELECT ... FOR UPDATE` immediately before returning; policy-decide reissue failures preserve inner error semantics and expose only `policyEvaluation`; prose/evaluator producer-reject without reasons returns deterministic `reject` instead of throwing. Phase 7 commit 1 adds a pure replay aggregator, a read-only DB loader, and a CLI report.

**Why:** Phase 7's design says autonomy must be measurable and separated by proposal kind. Phase 6's audit columns are enough to compute the first useful report over historical resolutions without rerunning generators or mutating state. Fixing the Phase 6 audit gaps first matters because a replay report is only as trustworthy as the rows it reads.

**Replay surface:** `src/canon/approval-policy-replay.ts` consumes `PolicyReplayRow[]` and returns a `PolicyReplayReport`. It also exposes `replayCandidatePolicy(frozenCases, policy)` for the next Phase 7 slice: evaluate frozen `ReviewProposalEnvelope`s against a candidate `ApprovalPolicy`, preserve the historical status as the outcome label, and feed the existing report builder. Metrics include policy approve/reject counts, auto precision, approve precision, reject precision, policy-vs-human agreement, intervention rate (`queue / total`), autonomous resolution rate (`resolved_by_kind=policy / total`), and canon auto-approve rate. Buckets are emitted by kind and by kind+risk so `canon_update`, `artifact_patch`, and `prose_edit` are never pooled.

**DB loader:** `src/db/approval-policy-replay.ts` is read-only. It unions resolved rows from `proposal_envelopes` and `canon_proposals`, filters to non-null `resolution_policy_decision`, and maps canon proposals to the synthetic replay kind/risk used in Phase 6 (`kind=canon_update`, `risk=high`). Supports `novelId`, `since`, and capped `limit`.

**Fixtures + CLI:** `docs/evals/approval-policy-replay-fixtures.md` documents the historical-row fixture shape and the frozen-envelope candidate replay shape. `docs/fixtures/approval-policy-replay/basic-policy-replay.json` includes four rows: artifact approve, prose reject, canon queue, and intentionally bad canon auto-approve. `docs/fixtures/approval-policy-replay/frozen-envelope-replay.json` includes four frozen `ReviewProposalEnvelope` cases for candidate-policy replay: low artifact approve, mechanical prose approve, high artifact reject, and high canon approve. `scripts/approval-policy-replay-report.ts` supports markdown or JSON output:

```bash
bun scripts/approval-policy-replay-report.ts --format markdown
bun scripts/approval-policy-replay-report.ts --novel <novelId> --format json
bun scripts/approval-policy-replay-report.ts --fixture docs/fixtures/approval-policy-replay/basic-policy-replay.json --check
bun scripts/approval-policy-replay-report.ts --frozen-fixture docs/fixtures/approval-policy-replay/frozen-envelope-replay.json --candidate-policy /tmp/policy.json --format json --check
bun scripts/approval-policy-replay-report.ts --check --min-rows 25 --min-auto-precision 0.98
```

`--check` exits non-zero when promotion thresholds fail. Defaults are deliberately conservative for the tracer: `minRows=1`, `minAutoPrecision=0.95`, `maxCanonAutoApproveRate=0`.

**Alternatives rejected:**

- *Start with generator replay over frozen artifacts.* Deferred. That is the next Phase 7 slice, but the audit-row report is cheaper, immediately useful, and validates the metric vocabulary before replaying producers.
- *Pool all proposal kinds into one precision number.* Rejected by design acceptance. Canon risk is qualitatively different from prose and artifact patches; pooling would hide unsafe canon behavior behind high-volume low-risk prose edits.
- *Make the report write to a table now.* Rejected for tracer-bullet scope. The first useful property is read-only reproducibility; persistence/CI can wrap the CLI once the report shape stabilizes.

**Tests:** New pure replay tests cover status-to-decision mapping, kind/risk separation, auto precision, canon auto-approve promotion failure, undefined-threshold default preservation, insufficient-row failure, candidate-policy replay over frozen envelopes, and markdown rendering. DB loader tests cover proposal_envelopes + canon_proposals union plus `since`/`limit` filters and skip when Postgres is unreachable. CLI tests cover defaults, filters, thresholds, historical row fixture mode, frozen-envelope candidate mode, and invalid format. Focused Phase 7 run: 22 pass / 2 DB-skipped. Touched suite: 53 pass / 70 skip. Server TypeScript clean. Historical fixture smoke with `--check` intentionally exits 1 because the seed includes a bad canon auto-approve row.

**Ongoing implications:**

- **Phase 7 is now open and has its first measurable artifact.** A policy change can be accompanied by a replay report today, either over historical audit rows or over a frozen-envelope candidate fixture.
- **Next Phase 7 slice:** replay proposal generators against frozen artifacts/gold fixtures, then compare generated proposal output plus candidate policy decisions to the historical audit-row baseline from this tracer.
- **CI/promotion wiring remains pending.** The CLI has `--check`; the repo still needs a policy-change guard that requires running it when `approval-policy` or policy config changes.

### L75 — Phase 7 autonomous loop: guard, downstream metrics, generator replay harness (2026-05-04)

**Decision:** Continue Phase 7 through the items that do not require an operator/product call. Add a local promotion guard, expand replay reports with downstream-impact metrics, and expose a pure generator replay harness. No runtime apply behavior changes.

**Why:** Phase 7 acceptance has three mechanical requirements: policy changes need replay evidence, metrics must include downstream impact, and autonomy must be testable against frozen historical artifacts without touching production state. These can be advanced without choosing a new policy or selecting production thresholds beyond the conservative defaults already in L74.

**Promotion guard:** New `scripts/approval-policy-promotion-guard.ts` detects changed approval-policy behavior files (`src/canon/approval-policy.ts` plus the policy application routes) and requires `--report <report.json>`. The report must be JSON, `totalRows > 0`, include non-empty `byKind`, and have `promotion.pass === true`. `package.json` exposes it as:

```bash
bun run policy:promotion-guard -- --report /tmp/approval-policy-replay-report.json
```

This is local/CI-ready wiring. The repo currently has no `.github/workflows`, so external CI YAML remains optional integration work rather than a Phase 7 code blocker.

**Downstream metrics:** `PolicyReplayRow` now carries optional post-resolution observations: `downstreamCheckerFired`, `downstreamEditChurn`, and `downstreamCanonConflict`. Buckets now report approval rate, checker-fire rate, average edit churn, and Canon conflict rate alongside existing precision/intervention metrics. DB replay rows explicitly populate these as `null` until a downstream loader/source can observe them; fixtures may provide values now.

**Generator replay harness:** `replayProposalGenerator(cases, policy, generate)` runs an injected proposal generator against frozen case input, matches generated envelopes to expected envelope ids, evaluates matched envelopes through `evaluatePolicy`, and returns both the normal `PolicyReplayReport` and generator drift signals (`missingExpected`, `unexpectedGenerated`, generated/matched counts). It is pure and read-only: no DB writes, no proposal apply, no mutable artifact reload.

**Generator replay CLI + seed:** `scripts/approval-policy-replay-report.ts` now supports:

```bash
bun scripts/approval-policy-replay-report.ts --generator artifact-patch-envelope --generator-fixture docs/fixtures/approval-policy-replay/artifact-patch-generator-replay.json --candidate-policy /tmp/policy.json --format json --check
bun scripts/approval-policy-replay-report.ts --generator lint-to-prose-edit --generator-fixture docs/fixtures/approval-policy-replay/lint-to-prose-edit-generator-replay.json --candidate-policy /tmp/policy.json --format json --check
bun scripts/approval-policy-replay-report.ts --generator editorial-beat-coverage --generator-fixture docs/fixtures/approval-policy-replay/editorial-beat-coverage-generator-replay.json --candidate-policy /tmp/policy.json --format json --check
```

The checked-in artifact seed fixture pins a pure `buildArtifactPatchEnvelope` character update. The checked-in lint seed fixture pins deterministic `FILLER_PHRASE`, `REDUNDANT_BODY`, and `SAID_BOOKISM` lint issues plus expected prose-edit envelope ids. The checked-in beat-coverage fixture injects frozen `BeatCoverageLlmOutput` into the producer, so replay never calls a live model while still exercising the envelope builder. The report includes normal replay metrics plus a `generatorReplay` block with case/generated/matched counts and drift lists. In generator mode, `--check` now fails on either promotion failure or generator drift (`missingExpected` / `unexpectedGenerated` non-empty).

**Tests:** Added focused guard tests plus replay tests for downstream metrics and generator replay, including artifact patch envelope replay, deterministic lint-to-prose-edit replay, and frozen-output editorial beat-coverage replay. Focused Phase 7/guard run: 32 pass / 2 DB-skipped. Touched suite: 71 pass / 70 skip under DB-backed harness gates. Server TypeScript clean.

**Ongoing implications:**

- **Policy-change replay gate is now available locally.** CI can call the guard directly once an external workflow exists.
- **Generator replay now has a harness but not concrete producer fixture suites.** The next no-product-decision work is adding one fixture suite per generator (`lint-to-prose-edit`, beat coverage, artifact adjuster) as frozen inputs become available.
- **Downstream-impact metrics have schema/harness support but not DB population.** Loader wiring should wait for stable sources for checker fires, edit churn, and Canon conflicts.

### L76 — Phase 7 policy decisions: persistence timing, posture, outcomes, thresholds (2026-05-04)

**Decision:** Adopt the conservative Phase 7 rollout posture and implement the mechanical parts now. Proposal producers persist envelopes at existing review checkpoints instead of applying inline: `lint-to-prose-edit` after deterministic lint/check pass, `editorial-beat-coverage` after draft generation/checking with frozen LLM output in replay, and artifact patch envelopes when the adjuster proposes changes. Runtime defaults stay manual. Assisted autonomy is limited to deterministic mechanical prose edits. Autonomous approval is allowed only in isolated scheduler/eval lanes for low-risk `artifact_patch` / `prose_edit`; Canon remains manual by default through `manualKinds: ["canon_update"]`.

**Why:** This keeps the review-card architecture load-bearing. The system can measure policy behavior and generator drift before changing production apply behavior, and Canon cannot be made unsafe by a broad autonomous-mode toggle.

**Downstream outcome source:** Add `sql/042_proposal_resolution_outcomes.sql` plus `src/db/proposal-resolution-outcomes.ts`. The table is keyed by `(source_table, proposal_id)` and stores post-resolution observations (`downstream_checker_fired`, `downstream_edit_churn`, `downstream_canon_conflict`) separately from immutable resolution audit rows. `src/db/approval-policy-replay.ts` now left-joins this table for both `proposal_envelopes` and `canon_proposals`, so replay metrics use explicit observations when present and remain `null` when no checker/edit/canon source has populated them yet.

**Promotion tiers:** `scripts/approval-policy-replay-report.ts` now accepts `--tier dev|assisted|autonomous`. `dev` remains the tracer default (`minRows=1`, `minAutoPrecision=0.95`, `maxCanonAutoApproveRate=0`). `assisted` requires `minRows=25`, `minAutoPrecision=0.95`, and zero Canon auto-approve. `autonomous` requires `minRows=100`, `minAutoPrecision=0.98`, and zero Canon auto-approve. Explicit threshold flags still override the tier. This keeps local iteration cheap while making promotion intent explicit.

**CI posture:** Keep enforcement local/CI-ready for now via `policy:promotion-guard`; do not add external workflow YAML until report source and tier choice are stable for the deployment environment.

**Tests:** Focused update run: replay CLI + pure replay + outcome DB tests report 35 pass / 4 DB-skipped. Full touched-suite run: 73 pass / 72 DB-skipped. Server TypeScript clean.
