...
**Penalty Judge:** GPT-OSS 120B (Groq) — `penalty-judge` in `models/roles.ts` (issue counting)
...
**Agent prompt changes** (manual or automated):
```
[agent:fact-extractor] completeness 3.0 → 4.7 (+1.7)

Added CRITICAL EXTRACTION REQUIREMENTS section targeting sensory
details, named specifics, ritual components.

extraction/completeness: 4.7/10 | 2 samples x 2 runs
experiment: #27
improver: kimi-k2
```

**Reverted attempts** (automated loop records failures too):
```
[agent:fact-extractor] revert: completeness 4.3 → 4.3 (+0)

Reverted: Added mandatory fight sequence detail extraction.

extraction/completeness: no improvement | iteration 2/5
experiment: #27
```

**Infrastructure/script changes**:
```
[infra] Add filtering to all benchmarks for tight iteration

New env filters: BENCHMARK_SEEDS, BENCHMARK_SAMPLES, BENCHMARK_FIXTURES,
BENCHMARK_AGENT. Data surface per cycle reduced 60-93%.
```

**Benchmark baseline runs**:
```
[baseline] Establish planning benchmark baselines

Beat Specificity 8.0  Dialogue Cues 5.0  Emotional Arc 8.8
5 seeds x 2 runs | GPT-OSS 120B judge
```

**Model/role changes**:
```
[roles] Set DeepSeek V3.2 as pairwise judge

5/5 consistent discrimination, 0% position bias (exp #24).
Cheapest of the top-tier pairwise judges.
```

The improvement loop auto-commits each kept change AND each reverted attempt so `git log src/agents/*/prompt.md` shows the full trail of what was tried, what worked, and what didn't.

Cost per iteration cycle: ~$0.02-0.05 (tight) / ~$0.06-0.17 (full validation).
