...
- **Validation** (`src/phases/validation.ts`): **rewriter** (Cerebras Qwen 235B) reruns chapters that fail deterministic validation. **tonal-pass** runs *after* validation converges, once across all approved chapters — per-paragraph LoRA voice rewrite, dialogue-only paragraphs skipped. Currently: Together AI Qwen 3.5 9B + howard-tonal-v3 (legacy). Validated replacement: W&B Inference `howard-tonal-v4-sft-resume:v8` (exp #98 — superior on all metrics, 3× faster).
...
- **Adherence checker** — per-beat: deterministic (character presence, word count, dialogue) + 4-call LLM verification (events/setting/tangent/character in parallel) on **W&B Qwen3-14B-Instruct**. Retries the beat up to `pipeline.maxBeatRetries` if adherence fails.
