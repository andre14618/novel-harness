...
- **Writing layer — cadence/tone imitation.** Matches prose rhythm, voice, dialogue patterns, sensory density of a target genre/author. **This is the highest-impact fine-tune use case** — cadence transfers via model weights in ways prompts/few-shot cannot match. Voice LoRAs (Salvatore v3/v4) are the canonical case; each genre gets its own when corpus is available. Context engineering (exampleLines, structural priors, character snapshots) supports voice but does not replace the fine-tune.
...
**Strategic goal:** semi-autonomous novel writing. Author shapes the plan; the harness drafts. Offline-capable (no API dependencies at inference) via small fine-tuned models running locally. Small-model POCs (Qwen3-1.7B, Qwen3-4B, Llama-3.2-3B) are both a cost/latency play AND a learning exercise in getting the most out of small models via focused fine-tuning.
