...
- **Planning layer — structural imitation.** Imitates the structure of successful storytelling (beat rhythms, cluster patterns, opener/closer rules, scene sizes, tension curves). Extracted from proven corpora (Salvatore reference implementation), rendered into planner constraints via `WRITER_GENRE_PACKS`. Long-term: robust human-in-the-loop planning stage where the author shapes world/character/arc commitments.
- **Writing layer — cadence/tone imitation.** Matches prose rhythm, voice, dialogue patterns, sensory density of a target genre/author. **Status 2026-04-21:** voice-LoRA track is FROZEN (not retired) per the LoRA-track pivot — see `docs/decisions.md` "Voice-LoRA track frozen; DeepSeek V3.2 base becomes the strategic writer target" + `docs/retrospectives/2026-04-21-lora-track-evidence.md`. The 14B-base LoRA approach (Salvatore v3/v4) hit structural limits: V1-prose anchor on rewrite, corpus-vocabulary bleed-through, stagnation on context-engineering levers. The harness's strategic writer target is now DeepSeek V3.2 base + prompt-level voice shaping; the `voice-shaping-ablation-v1` charter is the first experiment under the new architecture. Existing Salvatore v4 remains in production via `WRITER_GENRE_PACKS` fantasy routing until a voice-shaping arm produces a direct replacement candidate. The "highest-impact fine-tune use case" claim was specifically about voice-LoRA on a 14B base; at larger-base scales the in-context-learning regime is fundamentally different, so prompt+pipeline voice shaping is tested before returning to fine-tuning.
...
**Strategic goal:** semi-autonomous novel writing. Author shapes the plan; the harness drafts. **Offline-capable writing is NO LONGER a primary constraint** as of the 2026-04-21 pivot — the strategic writer is DeepSeek V3.2 base via API. The offline-capability thesis ("run small fine-tuned models locally") is preserved as an option for checkers (narrow tasks, small-model POCs remain viable per decisions.md 2026-04-XX "Small-model checker POC") but is NOT load-bearing for the writer layer. Small-model checker POCs (Qwen3-1.7B, Qwen3-4B, Llama-3.2-3B) continue as cost/latency/learning exercises on narrow checker tasks, not as writer candidates.

**Leak detection is per-writer, not universal.** Salvatore LoRA leaks Drizzt/Mithril Hall; future Gemmell/Cook LoRAs would each leak their own corpus vocabulary. Per-writer leak adapters are the right shape, maintained as part of the cost of genre-voice fine-tuning.
...
- **Drafting** (`src/phases/drafting.ts`):
  ...
  - **chapter-plan-checker** — **DeepSeek V3.2 base** (swapped from W&B SFT adapter 2026-04-18). Focused on cross-beat properties only: setting coherence, emotional arc direction, major plot contradictions. The SFT adapter `chapter-plan-checker-v2:v1` was retired after a dual-oracle audit (Sonnet + Codex gpt-5.4) found ~92% false-positive rate on real fantasy plans despite validated 96% accuracy on exp #178 — distribution drift between training scenarios and production output. DeepSeek base handles the narrow 3-question check natively. Deviations are beat-indexed (schema: `{description, beat_index: number|null}`) and route to **beat-targeted rewrites** inside the chapter attempt (`pipeline.maxChapterPlanRewritePasses=2`). On exhaustion, escalates once per chapter to `chapter-plan-reviser`. SFT recalibration deferred to `docs/todo.md` low-priority.
  - **chapter-plan-reviser** — **DeepSeek V3.2 base** (2026-04-19). Only invoked once per chapter when chapter-plan-checker's targeted-rewrite settle loop exhausts. Takes original plan + current prose + persistent unresolved issues; returns the smallest beat-list edit that would make the issues satisfiable. Output matches `chapterBeatsSchema` so the revised plan drops straight into `outline.scenes`. Post-revision sanity checks reject plans below beat-count floor or with newly introduced characters. Accepted revisions persist to `chapter_outlines` via `saveChapterOutline()`. Telemetry logged to `chapter_revisions` (sql/028) with outcome enum: `accepted | rejected_beat_floor | rejected_new_characters | error | skip_already_revised | skip_duplicate_sig | skip_no_beat_state`.
  - **continuity** — **W&B `continuity-v2:v1`** (Qwen3-14B SFT, 253 Sonnet-labeled pairs from 39 scenarios). 2 parallel decomposed agents (`continuity-facts` + `continuity-state`). Called per chapter in `drafting.ts`. Not a focus area — context-engineering shifts mean this checker sees much smaller inputs now than at its original design (the historical "7k token dump" framing is stale). De-emphasized in current roadmap; may be further retired as beat-level adherence + hallucination checks subsume its role.
...
- **Adherence checker** — per-beat: deterministic (character presence only) + single LLM call (events+attribution) on **W&B Qwen3-14B-Instruct**. Events failure triggers targeted rewrite (prose + specific issues passed back to writer). Setting and tangent calls removed — zero production signal. Dialogue deterministic check removed — false positive for intentionally silent scenes. Word-count gate removed 2026-04-16. See `docs/retry-surface-audit.md`.
- **Chapter plan checker** — per-chapter: DeepSeek V3.2 base (swapped from SFT 2026-04-18 after ~92% FP rate on real fantasy plans). Checks cross-beat properties: setting coherence, emotional arc direction, major plot contradictions. Emits beat-indexed deviations that route to targeted beat rewrites; escalates once per chapter to `chapter-plan-reviser` on rewrite-budget exhaustion. Telemetry via `chapter_revisions` table + `GET /api/novel/:id/revisions` + Studio `RevisionsPanel`.
- **Continuity checker** — per-chapter: **W&B `continuity-v2:v1`** (Qwen3-14B SFT) — 2 parallel decomposed calls (facts + state) check against world state tables. De-emphasized in current roadmap; context-engineering shifts have reduced its input size substantially from the original design. Beat-level adherence + hallucination checks are subsuming its role.
- **Hallucination checker (v3 architecture, candidate)** — decomposed into two parallel adapters: (a) **`halluc-ungrounded-v2`** — corpus-agnostic grounded-context check against speakers/brief/world_bible, 14B W&B adapter; (b) **`halluc-leak-salvatore-v1`** — per-writer Salvatore-corpus-vocabulary check, prose-only input, 14B W&B adapter. Combined via OR logic; aggregated issues feed drafting.ts retry. Natural-val combined: 77.8%/85.4%/81.4% (precision/recall/F1) — matches v1 baseline F1 with different trade-off. Status `candidate` in `adapter_registry` — not wired into drafting.ts yet. v2 kitchen-sink adapter REJECTED (distribution shift from pure-synth training). See `docs/decisions.md` 2026-04-18 "Hallucination-checker v3 two-adapter architecture."
