...
- **Validation** (`src/phases/validation.ts`): **rewriter** (**DeepSeek V3.2**) reruns chapters that fail deterministic validation. **tonal-pass auto-run disabled** — Howard methodology retired 2026-04-16; voice now lands at generation time via per-genre voice LoRAs. On-demand `POST /api/novel/:id/tonal-pass` still works for experimentation on existing novels. Adapter `howard-tonal-v4-sft-resume:v8` retained but not auto-invoked.
