...
// `./scene-checks` MUST re-export its full shape — otherwise, when
// `src/phases/scene-checks.test.ts` loads later in the same `bun test src/`
...
// `./scene-checks.mock-shape.ts` so drift is one place, not four.
