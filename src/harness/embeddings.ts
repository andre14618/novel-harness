...
export async function embedChapterData(novelId: string, chapterNum: number): Promise<{ embedded: number }> {
  ...
  if (facts.length > 0) {
    const texts = await Promise.all(facts.map((f: any) => buildFactEmbedText(f.category, f.fact)))
  ...
  if (events.length > 0) {
    const texts = await Promise.all(events.map((e: any) => buildEventEmbedText(e.event, e.location, e.participants_json as string[], e.consequences)))
    const embeddings = await getEmbeddings(texts)
  ...
  if (summaries.length > 0) {
    const texts = await Promise.all(summaries.map((s: any) => buildSummaryEmbedText(s.chapter_number, s.summary, s.key_events_json as string[], s.emotional_state)))
    const embeddings = await getEmbeddings(texts)
  ...
  if (states.length > 0) {
    const texts = await Promise.all(states.map((s: any) => {
  ...
  if (rels.length > 0) {
    const texts = await Promise.all(rels.map((r: any) => buildRelationshipEmbedText(r.character_a, r.character_b, r.trust_level, r.dynamic, r.tension, r.recent_shift)))
    const embeddings = await getEmbeddings(texts)
  ...
  if (knowledge.length > 0) {
    ...
    const texts = await Promise.all(knowledge.map((k: any) => buildKnowledgeEmbedText(charMap[k.character_id] ?? k.character_id, k.source, k.knowledge, k.is_false)))
    const embeddings = await getEmbeddings(texts)
