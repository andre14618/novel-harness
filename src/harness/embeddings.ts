...
export async function embedChapterData(novelId: string, chapterNum: number): Promise<{ embedded: number }> {
  ...
  if (states.length > 0) {
    const texts = await Promise.all(states.map(s => {
  ...
  if (rels.length > 0) {
    const texts = await Promise.all(rels.map(r => buildRelationshipEmbedText(r.character_a, r.character_b, r.trust_level, r.dynamic, r.tension, r.recent_shift)))
    const embeddings = await getEmbeddings(texts)
