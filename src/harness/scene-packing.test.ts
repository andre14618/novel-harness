...
import { packChapterScenesToBudget } from "./scene-packing"
...
test("merged beat description uses bullet format with 1-indexed source range", () => {
  ...
  const merged = result.outline.scenes.find(scene =>
    scene.description.startsWith("Packed from source scenes "),
  )
  expect(merged).toBeDefined()
  expect(merged!.description).toMatch(/^Packed from source scenes \d+(-\d+)?:\n/)
...
test("handles empty scene array as a clean no-op", () => {
  const result = packChapterScenesToBudget(chapter([], 1500))
  expect(result.audit.noOp).toBe(true)
  expect(result.audit.sourceSceneCount).toBe(0)
  expect(result.audit.packedSceneCount).toBe(0)
  expect(result.audit.openerPreserved).toBe(true)
  expect(result.audit.endpointPreserved).toBe(true)
