...
export { config }
export { schema, sceneExpansionSchema, type SceneExpansion, type ChapterScenePlan } from "./schema"
export { buildContext } from "./context"

// Variant-runner seam: when PLANNING_SCENES_PROMPT_OVERRIDE is set (absolute path),
...
const overridePath = process.env.PLANNING_SCENES_PROMPT_OVERRIDE?.trim()
const promptPath = overridePath && overridePath.length > 0 ? overridePath : defaultPromptPath
if (overridePath && overridePath.length > 0) {
  console.error(`[planning-scenes] PROMPT OVERRIDE in effect: ${promptPath}`)
}
