...
- **`lockedCharacters[]`** — characters the author named or described. For each: `name`, optional `role` (protagonist/antagonist/supporting/mentor/etc. — use whatever term the author used), `mustHaveTraits[]` (specific traits they called out), `mustHaveArc` (the trajectory they want, if stated). Do NOT add generic traits the author didn't mention. **`name` MUST be a proper name** — never "the cannibal", "the scholar", "the mentor", or a role word. If the author only referred to a character by archetype ("the cannibal king"), invent a proper name fitting the premise/genre and put the archetype description in `role` and `mustHaveTraits`. A character with no proper name gets filtered out by downstream validation.
...
- **`storyThreads[]`** — named long-running plot, relationship, mystery, promise, or character-arc threads the author explicitly wants tracked. Use `label`, optional `description`, optional `kind`, and optional `threadId` only if the author supplied an ID-like label. Do not invent threads from generic genre expectations.
- **`storyDebts[]`** — promises/questions/obligations that should be opened, progressed, or paid off later. Use `promiseText`, optional `threadId`, optional `storyDebtId`, optional `openedByChapter`, optional `expectedPayoffChapter`, and optional `payoffPolicy`. Extract only author-stated story debts.
- **`storyPayoffs[]`** — explicit payoff targets the author wants landed. Use `payoffText`, optional `storyDebtId`, optional `threadId`, optional `payoffId`, and optional `targetChapter`.
