...
describe("renderBeatContext + scene contract", () => {
  ...
  it("renders the SCENE CONTRACT block when sceneContract is populated", () => {
    ...
    expect(out).toContain("Use the listed outcome/consequence as the terminal landing")
    expect(out).toContain("Final paragraph check: a named character must do, choose, refuse, reveal, take, leave, enter, or commit on page")
    expect(out).toContain("If the scene ends with unanswered setup or a later confrontation")
  ...
  it("summarizes the full writer context surface, not only scene contracts", () => {
    ...
    expect(surface.surfaces.sceneEndpointLandingGuidance).toBe(false)
    ...
    expect(surface.counts.sceneContractEndpointFields).toBe(0)
  ...
  it("selects the drafting brief prompt and records payload telemetry", () => {
    ...
    expect(selected.draftingBriefTrace.sections.sceneEndpointLandingGuidance).toBe(false)
    ...
    expect(selected.draftingBriefTrace.counts.sceneContractEndpointFields).toBe(0)
  ...
  it("renders the scene-turn drafting brief floor without changing the budget mode", () => {
    ...
    expect(selected.userPrompt).toContain("Treat the listed outcome/consequence as the terminal landing")
    expect(selected.userPrompt).toContain("Final paragraph check: a named character does, chooses, refuses")
