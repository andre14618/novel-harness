...
1. **Find the latest benchmark results.** Query the Postgres database via `data/connection.ts`. Get the most recent run and check if a baseline exists. If the DB has no runs, tell the user to run `bun benchmark/prose/run.ts` first and stop.

2. **Query the benchmark DB.** Use the Postgres connection from `data/connection.ts`. Key queries:
