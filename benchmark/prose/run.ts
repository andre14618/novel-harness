...
async function main() {
  ...
  const allPenaltyScores: Array<{ seed: string; run: number; dim: Dimension; count: number; wordCount: number }> = []
  ...
  // ── Penalty results ──
  console.log(`\n  Dimensions (issues | per 1k words):`)
  const penaltyStats: Array<{ dim: Dimension; avg: number; std: number; normAvg: number; normStd: number }> = []
  for (const dim of DIMENSIONS) {
  ...
  for (const seed of seeds) {
    ...
    const penStr = DIMENSIONS.map(dim => {
      const dimScores = seedPenalties.filter(s => s.dim === dim)
      return `${DIMENSION_LABELS[dim]}:${mean(dimScores.map(s => abs(s.count))).toFixed(1)}`
    }).join(" ")
    const seedPenAvg = mean(seedPenalties.map(s => abs(s.count)))
    console.log(`    ${seed.name.padEnd(24)} ${penStr} (avg ${seedPenAvg.toFixed(1)}, ${seedWords.toFixed(0)}w)`)
  }
