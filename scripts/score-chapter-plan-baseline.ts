/**
 * Chapter-plan-checker baseline ladder: Llama 8B vs Qwen3-14B base vs gpt-oss-120b vs Qwen 235B.
 *
 * The head-to-head we never ran. exp #107 measured base 14B against gpt-oss-120b
 * (the production chapter-plan-checker model) on 80 synthetic pairs and got 58%
 * agreement — but Qwen 235B was never in that ladder. So we don't actually know
 * whether gpt-oss-120b is the right SFT teacher for chapter-plan-checker, or
 * whether 235B (which is the production model on every other analytical agent)
 * would be a stronger label source.
 *
 * This script answers two questions:
 *   1. Where does base 14B sit on agreement-vs-deterministic-label across the
 *      full ladder? (existing 58% number was vs gpt-oss outputs, not labels)
 *   2. Is gpt-oss-120b actually better than Qwen 235B on this task? If 235B beats
 *      gpt-oss against the synthetic gold, the SFT teacher should change.
 *
 * Reads:  lora-data/chapter-plan-checker-pairs.jsonl (80 pairs from generate-chapter-plan-data.ts)
 * Writes: tuning_experiment row with full per-model + per-variant stats.
 *
 * Variants from the pairs file:
 ...
 *     bun scripts/score-chapter-plan-baseline.ts
 *   ... --sample 24   (random sample for a quick smoke run — 3 per variant)
 */
...
interface VariantStats {
  ...
  falsePass: number   // label FAIL, model PASS  (over-permissive)
  falseFail: number   // label PASS, model FAIL  (over-strict)
}
...
async function main() {
  ...
  const expId = await createTuningExperiment(
    "baseline",
    `Chapter-plan-checker base-model ladder: Llama 8B vs Qwen3-14B vs gpt-oss-120b vs Qwen 235B (${pairs.length} pairs)`,
    {
      pairs: pairs.length,
      ...
      hypothesis: "gpt-oss-120b is the production chapter-plan-checker model and has been the assumed SFT teacher, but it has never been benchmarked against Qwen 235B on the deterministic labels. Two questions: (1) does gpt-oss-120b actually beat Qwen 235B on chapter-plan-checker? If 235B wins, the SFT teacher should change. (2) Where does base Qwen3-14B sit on absolute agreement-vs-label, not just agreement-vs-gpt-oss? The exp #107 number (58%) was vs gpt-oss outputs and conflated 'wrong vs label' with 'disagrees with teacher'.",
      relatedExperiments: [107],
    },
  ...
  // Concurrency: 3 pairs in flight, 4 models per pair fired in parallel = net 12.
  ...
  // ── Pairwise teacher comparison: gpt-oss-120b vs Qwen 235B head-to-head ──
  // The whole point of the script. Where do they disagree, and on what variants?
  console.log("\nTEACHER HEAD-TO-HEAD: gpt-oss-120b vs Qwen 235B")
  const gptStats = allStats.get("gptoss120b")!
  const qwenStats = allStats.get("qwen235b")!
  const gptOverall = pct(gptStats.overall.agree, gptStats.overall.agree + gptStats.overall.disagree)
  const qwenOverall = pct(qwenStats.overall.agree, qwenStats.overall.agree + qwenStats.overall.disagree)
  console.log(`  gpt-oss-120b overall: ${gptOverall}%`)
  console.log(`  Qwen 235B    overall: ${qwenOverall}%`)
  console.log(`  Δ (235B − gpt-oss):   ${qwenOverall - gptOverall >= 0 ? "+" : ""}${qwenOverall - gptOverall}pp`)
  for (const variant of variants) {
    const g = gptStats.byVariant.get(variant)
    const q = qwenStats.byVariant.get(variant)
    if (!g || !q) continue
    const gpct = pct(g.agree, g.agree + g.disagree)
    const qpct = pct(q.agree, q.agree + q.disagree)
    const delta = qpct - gpct
    const arrow = delta > 0 ? "↑" : delta < 0 ? "↓" : "="
    console.log(`  ${variant.padEnd(22)} gpt-oss=${String(gpct).padStart(3)}%  235B=${String(qpct).padStart(3)}%  ${arrow}${Math.abs(delta)}pp`)
  }
  ...
  const conclusion = JSON.stringify({
    pairs: pairs.length,
    teacherComparison: {
      gptOss120b: gptOverall,
      qwen235b: qwenOverall,
      deltaPp: qwenOverall - gptOverall,
      verdict: qwenOverall > gptOverall + 3 ? "235B is the better teacher"
             : gptOverall > qwenOverall + 3 ? "gpt-oss-120b is the better teacher"
             : "peer-tier on this task — pick on cost/latency",
    },
    models: MODELS.map(m => {
      const s = allStats.get(m.key)!
      const o = s.overall
