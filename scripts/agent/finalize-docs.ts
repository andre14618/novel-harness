...
import { writeFinalizerPacket } from "./finalizer-packet"
...
function usage(): string {
  return [
    ...
    "  --packet <path>       Existing deterministic handoff packet to pass",
    "  --no-packet           Skip automatic handoff packet generation",
...
export function parseArgs(argv: string[]): FinalizeDocsArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === "--packet") out.packetPath = argv[++i] ?? null
    else if (a === "--no-packet") out.noPacket = true
  ...
  if (out.packetPath === "") throw new Error("--packet requires a value")
  if (!out.message?.trim()) out.message = `[docs] finalize ${laneIdFromPath(out.lanePath)} documentation`
  return out
}

export function createPacketForFinalize(args: FinalizeDocsArgs): string | null {
  if (args.noPacket) return null
  if (args.packetPath) return args.packetPath
  const packet = writeFinalizerPacket({
...
export function buildPrompt(args: FinalizeDocsArgs): string {
  ...
  return [
    ...
    `- Deterministic handoff packet: ${args.packetPath ?? "(not supplied; build one before finalization unless explicitly skipped)"}`,
    "",
    "Required behavior:",
...
function main(argv: string[]): number {
  ...
  const packetPath = createPacketForFinalize(args)
  const opencodeArgs = buildOpencodeArgs(args)
  if (args.dryRun) {
    if (packetPath) console.log(`[finalize-docs] wrote deterministic handoff packet: ${packetPath}`)
