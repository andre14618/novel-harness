...
SETTING DETAILS:
The Frontier Outpost: A crumbling stone fort on the empire eastern edge. Walls pitted by decades of corrosive sand. Skeleton garrison of 30 soldiers, most of them disciplinary cases. Water rations are deliberately kept low.`
...
// Try Groq first, then any provider as fallback
const configs = [
  { label: "Llama 3.3 70B (Groq)", model: "meta-llama/llama-3.3-70b-instruct", provider: { order: ["Groq"], allow_fallbacks: false } },
  { label: "Llama 3.3 70B (any provider)", model: "meta-llama/llama-3.3-70b-instruct", provider: undefined },
]
...
for (const cfg of configs) {
  ...
  for (const t of tests) {
    ...
    try {
      ...
      if (data.error) { console.log(`FAIL  API error`); continue }
      ...
      console.log(`${zodResult.success ? "PASS" : "FAIL"}  ${(elapsed / 1000).toFixed(1)}s  ${tps} tok/s  tokens=${usage.completion_tokens}`)
